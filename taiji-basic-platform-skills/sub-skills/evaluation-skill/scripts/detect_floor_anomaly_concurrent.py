...
# ============================================================
# TOOL_CALL_ANOMALY 检测（完整对齐 detect_toolcall_anomaly.py）
# 仅针对 Agent 类 Bench
# ============================================================
...
def parse_trajectory_spans(traj_path):
    ...
    采用流式逐行解压 + 逐行解析，避免将整个文件加载到内存，
    峰值内存 ≈ 解压缓冲区(~64KB) + 累积的 parsed spans 结构化数据。

    每个元素: {turn, content, reasoning_content, tool_calls, finish_reason,
              tools, tool_choice, step_id, is_retry, span_id}
    """
    import hashlib

    line_iter, closables = _iter_lines_from_path(traj_path)
    if line_iter is None:
        return []

    # 流式解析：逐行读取，START span 只保留轻量元数据 (tools, tool_choice, fingerprint)
    start_metas = []   # [(tools, tool_choice, fingerprint), ...]
    update_spans = []  # [(span_id, msg_dict, finish_reason), ...]

    try:
        for line in line_iter:
            if 'openai_completion' not in line and 'choices' not in line:
                continue
            line = line.strip()
            if not line:
                continue
            ...
            if span_type == "START" and name == "openai_completion":
                ...
                if isinstance(start_attrs, dict):
                    ...
                    if isinstance(inp, dict):
                        ...
                        messages = inp.get("messages") or kw.get("messages") or []
                        if messages:
                            last_json = json.dumps(messages[-1], ensure_ascii=False, sort_keys=True)
                            sha1 = hashlib.sha1(last_json.encode("utf-8")).hexdigest()
                            fp = (len(messages), sha1)
                start_metas.append((tools, tool_choice, fp))
                ...
                span_id = (s.get("span_id", "")
                           or s.get("context", {}).get("span_id", ""))
                update_spans.append((span_id, msg, first_choice.get("finish_reason", "")))
    finally:
        if closables:
            for c in closables:
                try:
    ...
    # 配对 START 和 UPDATE，生成结果
    results = []
    last_fingerprint = None
    current_step_id = 0
    n_starts = len(start_metas)

    for turn_idx, (span_id, msg, fr) in enumerate(update_spans):
        ...
        tools = []
        tool_choice = ""
        if turn_idx < n_starts:
            tools, tool_choice, fp = start_metas[turn_idx]
        else:
            fp = (0, "")
...
def process_file(filepath, task_id, think_mode, debug_limit=None, detect_types=None):
    ...
    返回 (total_cases, subtype_counts, all_badcases)
    """
    ...
    return total, subtype_counts, all_badcases
...
def main():
    ...
    parser.add_argument("--write_mode", type=str, default="per_bench",
                        choices=["per_bench", "none"],
                        help="badcase 写入模式: "
                             "per_bench=每个bench检测完后批量写入(默认), "
                             "none=不写badcase文件")
    parser.add_argument("--workers", type=int, default=1,
                        help="并发工作进程数，默认1（串行）。"
                             "设置 >1 时以 bench 为粒度并发执行检测（ProcessPoolExecutor）。")
    ...
    # ---- 筛选需要处理的 bench，预先排除不适用的 ----
    bench_tasks = []  # [(filepath, tag, safe_name), ...]
    for filepath in gz_files:
        ...
        if is_agent and "TOOL_CALL_ANOMALY" not in detect_types \
           ...
           and "THINK_TAG_ILLEGAL" not in detect_types:
            ...
            continue
        bench_tasks.append((filepath, tag, safe_name))
    ...
    def _write_badcases(badcases):
        """将 badcases 批量写入对应 subtype 文件"""
        for bc in badcases:
            ar = bc.get("anomaly_results", {})
    ...
    def _report_bench(tag, safe_name, total, subtype_counts):
        anomaly_count = sum(subtype_counts.values())
        suffix = f" (限{debug_limit})" if debug_limit and total >= debug_limit else ""
        print(f"  {tag} {safe_name} ... {total} cases{suffix}, {anomaly_count} anomalies")
        if subtype_counts:
    ...
    try:
        if workers > 1:
            ...
            with ProcessPoolExecutor(max_workers=workers) as executor:
                for filepath, tag, safe_name in bench_tasks:
                    future = executor.submit(
                        process_file, filepath, task_id, think_mode,
                        debug_limit, detect_types
                    )
                    futures[future] = (filepath, tag, safe_name)

                for future in as_completed(futures):
                    filepath, tag, safe_name = futures[future]
                    try:
                        total, subtype_counts, badcases = future.result()
                    except Exception as e:
                        print(f"  {tag} {safe_name} ... ERROR: {e}", file=sys.stderr)
                        continue
                    ...
                    if write_mode != "none":
                        _write_badcases(badcases)

                    _report_bench(tag, safe_name, total, subtype_counts)
        else:
            # ---- 串行模式 ----
            for filepath, tag, safe_name in bench_tasks:
                print(f"  {tag} {safe_name} ...", end="", flush=True)

                total, subtype_counts, badcases = process_file(
