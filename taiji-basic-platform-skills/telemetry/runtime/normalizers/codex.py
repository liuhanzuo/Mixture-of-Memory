# -*- coding: utf-8 -*-
"""Codex-family hook event normalizer."""
...
def normalize_codex_event(raw, agent_variant, config):
    original = first(raw, "hook_event_name", "hookEventName")
    normalized, outcome, scope = map_event(original)
    # Codex currently has no PostToolUseFailure hook and its PostToolUse input
    # does not reliably expose a failure status (for example, Bash `exit 7`
    # emits PostToolUse with an empty tool_response). Do not misreport these
    # events as successful merely because the hook fired.
