# coding=utf-8
# ---------------------------------------------------------------------------
# PORTED to transformers 5.5.4 / torch 2.10 / sm_100 (L20A/B200) — 2026-07-05
#
# This is a self-contained copy of the official MemoryLLM (YuWangX/memoryllm-8b-chat)
# modeling file, adapted from its transformers 4.43-era assumptions so it loads and
# runs on the local B200/L20A .venv (torch 2.10.0+cu128 + transformers 5.5.4).
#
# All adaptations are tagged with the marker "# PORT:" so they are easy to audit
# against the original /tmp/mllm_modeling.py.  Adaptations made:
#   1. config.rope_theta was dropped as a top-level attr in tf5 (folded into
#      rope_scaling). _init_rope() / __init__ now read it robustly.
#   2. DynamicCache.from_legacy_cache / to_legacy_cache were removed in tf5;
#      compat shims are installed at import time.
#   3. Cache.get_max_length() -> get_max_cache_shape() in tf5.
#   4. PreTrainedModel no longer subclasses GenerationMixin in tf5; the causal-LM
#      classes now inherit GenerationMixin explicitly so .generate() works.
#   5. The flash-attn import is made lazy so attn_implementation="sdpa" works on
#      sm_100 without flash_attn installed.
#   6. Removed two stray ipdb/typo debug traps (jpdb, ipdb.set_trace) that would
#      crash on the exception paths.
#   7. RoPE inv_freq is a non-persistent buffer; from_pretrained under
#      accelerate/meta-device can leave it as garbage -> token-0 collapse
#      ("!!!!"/"MarcusMarcus").  MemoryLLM.from_pretrained now re-initialises
#      every rotary_emb.inv_freq after load.
#   8. transformers>=5's weight loader silently fails to populate most tensors of
#      this PEFT-wrapped custom architecture (embed_tokens / *.base_layer / lora_* /
#      memory / lm_head stay at random init, yet missing/unexpected keys are empty),
#      giving degenerate single-token generation.  MemoryLLM.from_pretrained now
#      force-reloads the full checkpoint state dict by exact name after the standard
#      load when given a local directory.
# ---------------------------------------------------------------------------
...
# PORT: flash-attn helper is imported lazily inside LlamaFlashAttention2.forward so
# that attn_implementation="sdpa" works on sm_100 without flash_attn installed.
try:
    from transformers.modeling_flash_attention_utils import _flash_attention_forward
except Exception:  # pragma: no cover - flash-attn utils unavailable
...
# PORT: PreTrainedModel no longer subclasses GenerationMixin in transformers>=5;
# import it so the causal-LM heads can inherit .generate() explicitly.
...
# PORT: transformers>=5 removed DynamicCache.from_legacy_cache / to_legacy_cache.
# MemoryLLM's forward still round-trips through the legacy tuple format, so install
# backwards-compatible shims once at import time.
...
# PORT: transformers>=5 no longer exposes a top-level config.rope_theta attribute
# (it is normalised into config.rope_scaling). This helper recovers the base
# frequency from either location so _init_rope() keeps working.
def _get_rope_theta(config, default=500000.0):
    ...
    if isinstance(rope_scaling, dict):
        ...
        if rope_scaling.get("base") is not None:
            return rope_scaling["base"]
    return default
...
class LlamaRotaryEmbedding(nn.Module):
    ...
    # PORT: inv_freq is a non-persistent buffer, so it is NOT in the checkpoint.
    # Under transformers>=5 + accelerate, from_pretrained builds the model on the
    # meta device; the buffer then stays meta/garbage after weight loading, giving
    # NaN cos/sin -> token-0 collapse ("!!!!"/"MarcusMarcus"). Recompute it after
    # load (see MemoryLLM.from_pretrained) using this helper.
    def _recompute_inv_freq(self, device=None):
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        # PORT: transformers>=5 drops the top-level config.rope_theta attribute
        # (it is folded into config.rope_scaling). Read it robustly.
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            # PORT: tf5 keeps both "type" and "rope_type" in rope_scaling; accept either.
    ...
    def forward(
        ...
        **kwargs,
        ...
        # PORT: removed stray `import ipdb; jpdb.set_trace()` debug trap (NameError bug).
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        # PORT: fail clearly if FA2 path is taken without flash-attn utils available.
        if _flash_attention_forward is None:
            raise RuntimeError(
                "attn_implementation='flash_attention_2' requires transformers flash-attn "
                "utils / flash_attn to be installed. Use attn_implementation='sdpa' instead."
            )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        if using_static_cache:
            # PORT: Cache.get_max_length() was renamed get_max_cache_shape() in tf5.
...
# PORT: transformers>=5 no longer mixes GenerationMixin into PreTrainedModel, so the
# causal-LM head must inherit it explicitly to expose .generate().
...
class MemoryLLM(LlamaForCausalLM):
    ...
    # PORT: recompute every rotary_emb.inv_freq buffer. Non-persistent buffers are
    # not in the checkpoint and can survive meta-device from_pretrained as garbage,
    # collapsing generation to token 0. Call this once after load / .to(device).
    def recompute_rope_inv_freq(self):
        # Determine a real (non-meta) target device from the loaded weights so that
        # device_map / low_cpu_mem_usage loads keep inv_freq on the same device.
        target_device = None
        ...
        for module in self.modules():
            if isinstance(module, LlamaRotaryEmbedding) and hasattr(module, "_recompute_inv_freq"):
                module._recompute_inv_freq(device=target_device)
                n += 1
        return n
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        # PORT: transformers>=5's from_pretrained weight loader does NOT correctly
        # populate this PEFT-wrapped custom architecture — most tensors (embed_tokens,
        # *.base_layer.weight, lora_*, memory, lm_head, ...) silently stay at their
        # random init values even though missing_keys/unexpected_keys are both empty
        # (only a few like model.norm.weight land). Generation then degenerates to a
        # single repeated token. As a robust fix, explicitly reload the full checkpoint
        # state dict with assign-in-place and verify a canary weight actually changed.
        try:
            n_reloaded = model._force_load_checkpoint_weights(pretrained_model_name_or_path)
            if n_reloaded:
                logger.info(f"[MemoryLLM-port] force-reloaded {n_reloaded} checkpoint tensors")
        except Exception as e:  # pragma: no cover - defensive
            logger.warning(f"[MemoryLLM-port] force weight reload failed: {e}")

        # Repopulate RoPE inv_freq (non-persistent buffer) so a subsequent .to(device)
        # allocates correct (not garbage) frequencies.
        try:
            n = model.recompute_rope_inv_freq()
            logger.info(f"[MemoryLLM-port] recomputed inv_freq on {n} rotary_emb modules")
        except Exception as e:  # pragma: no cover - defensive
            logger.warning(f"[MemoryLLM-port] inv_freq recompute failed: {e}")
        return model

    def _force_load_checkpoint_weights(self, model_path):
        """Load every tensor from the on-disk safetensors/pytorch checkpoint into the
        model, matching by exact parameter/buffer name. Returns the number of tensors
        that were actually copied. No-op (returns 0) if ``model_path`` is not a local
        directory containing a resolvable checkpoint (e.g. a bare HF repo id that was
        streamed) — in that case the standard loader result is kept as-is."""
        import os
        import glob

        if not (isinstance(model_path, (str, os.PathLike)) and os.path.isdir(str(model_path))):
            return 0
        ...
        with torch.no_grad():
            for name, tensor in state.items():
                ...
                if dst is None or tuple(dst.shape) != tuple(tensor.shape):
                    continue
                src = tensor.to(dtype=dst.dtype, device=dst.device)
                if dst.is_meta:
                    # replace meta param with a materialised copy on cpu
                    dst.data = src.clone()
                else:
                    dst.data.copy_(src)
                n += 1
    ...
    def update_memory_with_delta_memory(self, delta_memory):
        ...
        if self.initialized == 0:
            ...
            if delta_memory.shape[1] > self.num_tokens * self.num_blocks:
                # PORT: replaced `import ipdb; ipdb.set_trace()` with a hard error.
                raise ValueError(
                    f"delta_memory length {delta_memory.shape[1]} exceeds memory pool "
                    f"size {self.num_tokens * self.num_blocks}"
                )
...
# PORT: peft>=0.15 copies `base_model.prepare_inputs_for_generation` when wrapping a
# module with get_peft_model. MemoryLLM applies LoRA to `self.model` (a LlamaModel),
# which does not define that method in this file, so expose the causal-LM version on
# LlamaModel. Also mirror it onto PeftModelForCausalLM for the generate() path.
...
try:
    from peft import PeftModelForCausalLM as _PeftModelForCausalLM

    if not hasattr(_PeftModelForCausalLM, "prepare_inputs_for_generation"):
        _PeftModelForCausalLM.prepare_inputs_for_generation = (
