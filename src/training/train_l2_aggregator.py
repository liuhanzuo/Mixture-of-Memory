...
class L2AggregatorDataset(Dataset):
    ...
    def from_hf_dataset(
        ...
        支持的数据集:
          - "msc": Facebook Multi-Session Chat
    ...
    def _load_from_msc(self, max_samples: int = 500) -> None:
        """从 MSC 数据集加载并转换为 L2 训练格式。"""
        try:
            from datasets import load_dataset
        except ImportError:
            logger.error("[L2 Dataset] datasets 未安装，请运行: pip install datasets")
            logger.warning("[L2 Dataset] 退回到合成数据")
            self._generate_synthetic()
            return

        logger.info("[L2 Dataset] 从 HuggingFace 加载 MSC 数据集...")
        try:
            dataset = load_dataset("facebook/msc", "session_1", split="train",
                                   trust_remote_code=True)
            logger.info(f"[L2 Dataset] MSC 加载成功: {len(dataset)} 条对话")
        except Exception as e:
            logger.error(f"[L2 Dataset] MSC 加载失败: {e}")
            logger.warning("[L2 Dataset] 退回到合成数据")
            self._generate_synthetic()
            return
        ...
        try:
            from scripts.prepare_data import convert_msc_to_l2
            self.samples = convert_msc_to_l2(dataset, max_samples=max_samples)
        except ImportError:
            # 内联简化版转换
            self._convert_msc_inline(dataset, max_samples)

        logger.info(f"[L2 Dataset] 从 MSC 加载了 {len(self.samples)} 条训练样本")

        if not self.samples:
            logger.warning("[L2 Dataset] MSC 转换结果为空，退回到合成数据")
            self._generate_synthetic()

    def _convert_msc_inline(self, dataset, max_samples: int) -> None:
        """MSC → L2 内联转换 (无需 prepare_data 模块)。"""
        import random

        PREF_KW = ["prefer", "like", "love", "enjoy", "favorite", "hate", "always", "usually"]
        IDENT_KW = ["my name", "i am", "i'm", "i work", "i live", "my job"]
        TASK_KW = ["working on", "planning to", "going to", "need to", "currently", "project"]
        ...
        for example in dataset:
            ...
            if objects:
                self.samples.append({"messages": messages, "objects": objects})
...
class L2AggregatorTrainer:
    ...
    def _load_model_and_tokenizer(self) -> None:
        ...
        if model_path and not Path(model_path).is_absolute():
            ...
            if resolved.exists():
                logger.info(f"[L2 Trainer] 解析相对模型路径: {model_path} → {resolved}")
    ...
    def prepare_data(self) -> tuple[L2AggregatorDataset, L2AggregatorDataset | None]:
        ...
        if self.config.dataset_name:
            logger.info(f"[L2 Trainer] 使用 HuggingFace 数据集: {self.config.dataset_name}")
            train_dataset = L2AggregatorDataset.from_hf_dataset(
                self.config.dataset_name,
                max_samples=self.config.dataset_max_samples,
                max_seq_len=self.config.max_seq_len,
            )
            # HF 数据集模式下不使用单独的验证集，从训练集中分出 10%
            val_dataset = None
        else:
