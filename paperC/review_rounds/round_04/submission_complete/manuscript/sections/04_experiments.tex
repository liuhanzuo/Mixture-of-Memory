...
Every denominator below is over \emph{designated damaged} cells: arms structurally pruned and reported as damaged rungs of their family. The designation is fixed by construction, never by a measured score, so counts such as 14/15 and 10/15 are over exactly this set and cannot be inflated by selecting on outcome. Across such cells we report per-cell $\alpha=0.05$ decisions with no family-wise correction, so the headline counts are counts of per-cell decisions rather than simultaneously valid claims; the conclusions rest on the aggregate's near-unanimity (0/60 damaged cells clear their floor), not on one cell. Appendix~\ref{app:designated} states both.
