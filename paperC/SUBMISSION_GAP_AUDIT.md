...
| 4 | OpenBookQA content | `0.3635` **char** / `0.3680` token | token: **`0.368000`** exactly (OLMo-2 tokenizer, ties 242/500). **char: COULD NOT RECOMPUTE** | ⚠️ **token ✅ / char ✗ — this is E1**. Note the token null is *tokenizer-valued*: I got **`0.375833`** on the Llama-2 records for the same 500 items, so `0.3680` must always be quoted as "OLMo-2 tokenizer". |
...
**Score: 10 of 11 fully recomputable today; 1 (OBQA character unit) not, and its own
code says why.**
...
| Attempt OBQA **character** null | LOCAL CPU | **FAILED** — no `pyarrow`/`datasets` on conda, `.venv`, `python3.11`, `python3`. E1. |
