...
% summary/full-text pairing is recorded in paperC/C9_APPLIED_20260816.md.
% ---------------------------------------------------------------------------
\input{sections/03b_nulls}
\input{sections/09a_relocated}

\subsection{Per-cell MMLU-Pro read-out and the v2 re-sort}
\label{app:mmlupro-and-resort}

The two per-cell tables cited from the Results and Analysis section are placed here
for space. Table~\ref{tab:mmlupro} is the MMLU-Pro letter read-out for all 17
designated damaged cells against the always-A floor, supporting
``Wrong-null verdict flips at MMLU-scale power''. Table~\ref{tab:v2-resort} gives the
paired v1/v2 verdicts for the five cells whose labels the arm-conditional null moves,
supporting ``V2 re-sorts the 27-cell read-out''; Table~\ref{tab:v2-full} gives all 27
cells with Benjamini--Hochberg $q$-values over exactly that family, which is what
separates the surviving downgrade from the non-surviving upgrade. Every value in these
tables is unchanged from the version those subsections discuss.
...
Paths are relative to the repository's \texttt{paperC/} directory.
...
\toprule
Identifier & Artifact path (relative to \texttt{paperC/}) \\
\midrule
\textsf{E-A} & \texttt{tcodex\_out/EVIDENCE\_PACK.md}, Section A, re-derived from \texttt{evidence/SECOND\_MC\_BENCHMARK\_VERDICT.md} \\
\textsf{E-B} & \texttt{tcodex\_out/EVIDENCE\_PACK.md}, Section B; per-cell records in \texttt{evidence/mmlu\_scale\_power/} and \texttt{evidence/POWER\_WALL\_VERDICT.md} \\
\textsf{E-D} & \texttt{evidence/heal\_readout\_v2\_permutation\_null.json} (\texttt{tcodex\_out/EVIDENCE\_PACK.md}, Section D) \\
\textsf{E-E} & \texttt{evidence/R7\_BOOTSTRAP\_P\_FIX.md} (\texttt{tcodex\_out/EVIDENCE\_PACK.md}, Section E) \\
\textsf{E-F} & \texttt{evidence/second\_mc\_benchmark/} and \texttt{evidence/SECOND\_MC\_BENCHMARK\_VERDICT.md} (\texttt{tcodex\_out/EVIDENCE\_PACK.md}, Section F) \\
\textsf{E-H} & \texttt{evidence/construct\_nulls\_length\_unit.json} (\texttt{tcodex\_out/EVIDENCE\_PACK.md}, Section H) \\
\textsf{E-I} & \texttt{evidence/mmlu\_scale\_} \texttt{power/trunc\_fix\_} \texttt{before\_after.json} and \texttt{evidence/mmlu\_scale\_} \texttt{power/} \texttt{trunc\_audit\_1536.json}, emitted by \texttt{code/mmlu\_pro\_} \texttt{trunc\_fix\_compare.py} and \texttt{code/mmlu\_pro\_} \texttt{trunc\_audit.py} (\texttt{tcodex\_out/} \texttt{EVIDENCE\_PACK.md}, Section I) \\
\textsf{E-K} & \texttt{tcodex\_out/EVIDENCE\_PACK.md}, Section K; row-by-row map in \texttt{evidence/claim\_evidence\_map.tsv} \\
