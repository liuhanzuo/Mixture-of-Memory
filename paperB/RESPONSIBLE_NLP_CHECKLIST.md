...
- **B2 — Partially yes.** The anonymous artifact README states that upstream
  model, corpus, and benchmark licenses remain controlling and that the package
  redistributes neither weights nor benchmark text. Authors must confirm every
  upstream term in the submission form before upload.
...
- **C1 — Partially yes.** The paper reports model scale, GPU setup, training
  steps, and checkpoint provenance. Reliable historical per-run GPU-hours and
  total project compute could not be reconstructed; this is disclosed rather
  than estimated.
...
- **C4 — Partially yes.** Appendix B and the anonymous artifact provide the
  available PyTorch/Transformers/Datasets versions, evaluator snapshots,
  architecture reconstruction, score-level records, and checksums. Exact
  historical training reproduction remains impossible because seeds and a
  resumed loader offset were not recorded.
