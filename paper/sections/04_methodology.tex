...
would confine each retrieved chunk to attend only within itself---the query
still attends to all retrieved chunk hiddens and the sink, but the chunks can no
longer attend across one another---and this loss of cross-chunk interaction
collapses multi-fact disambiguation; we motivate this in \S\ref{sec:motivation}
and ablate it in \S\ref{sec:experiments} (Table~\ref{tab:crosschunk}). Because the pack
...
LoRA adapter ($r{=}32$, $\alpha{=}64$; $58$M trainable parameters, $0.71\%$ of
the backbone, applied only to the recompute layers $12$--$35$), for $4000$ steps
of single-node 8-GPU DDP over $66$M tokens of plain PG19~\citep{pg19} continuous
windows---\emph{no synthetic
...
\paragraph{No special pretraining is required.} We stress that CoMem's memory
architecture and inference-time operations require no parameter updates: the
depth partition, the retrieval readout, and the $j$ knob all operate on a stock,
frozen backbone. The flagship uses a lightweight self-distilled LoRA (above) to
push the usable split to $j{=}12$, while an adapter-free variant operates at a
shallower readout-safe split ($j{\le}9$ on Qwen3-8B); the LoRA is thus a light,
