#!/usr/bin/env bash
# 方案B FIFO b25 chunk512 — 零泄漏对照 (NOLEAK)
# 与 launch_mem_space_fifo_b25_chunk512_diskB.sh 完全相同,唯一改动:
#   --babilong_mix_fraction 0   (关闭 BABILong eval-split 数据混入)
# 用途: 隔离 FIFO 架构在 BABILong 上的真实能力
#   "脏 b25" qa5 0k=100/4k=87 vs 此干净版本的差值 = 泄漏贡献
...
RUN="mem_space_fifo_b25_chunk512_noleak"
mkdir -p logs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29801 \
