#!/usr/bin/env bash
# 方案B: FIFO hidden-state memory (MemoryLLM-style) — buffer100 chunk512 ablation arm
# Runs on H20 .58.245.174 (28.58.245.174, 8× H20 95GB, 盘B share_304376610, .venv)
# OOM fix v1: changed chunk_size 1024->512 from original buf100_diskB script (H20 95GB OOM with chunk1024)
# OOM fix v2 (2026-06-24): bptt_window 2->1 to reduce activation memory peak at step 30
# OOM fix v3 (2026-06-24): unfreeze_layers_from 16->24 (optimizer.step() OOM@step30, saves ~7GB Adam states)
# b100×chunk512 full_attn over 100+50 context chunks causes OOM on H20 95GB even with expandable_segments
# Experiment axis: buffer_length (b25/b50/b100 × chunk512); .196=b50 chunk512 base
# 2026-06-24
set -euo pipefail
...
RUN="mem_space_fifo_b100_chunk512"
mkdir -p logs
