#!/usr/bin/env bash
# Phase-8B v8-C — global slots with input-gate-only writeback on Llama-3-8B-Instruct.
#
# Purpose: v8-C makes global slots use only the input gate for writing:
#   slot ← g_in · tanh(s_new)
# No forget term — pure write register with amplitude control via g_in.
# Compared to v7 (replace=True, no gate), this adds learnable amplitude scaling
# but no memory retention from previous content.
#
# Launch (on B200 28.89.17.144 via wzc1 shared FS):
#   bash scripts/launch_phase8b_v8c_global_input_gate.sh
#
# Target node: B200 28.89.17.144 (8x L20A, 183 GiB)
# Outputs: outputs/babilong_sft_phase8b_v8c_global_input_gate_<timestamp>/
# Log:     logs/phase8b_v8c_global_input_gate_<timestamp>.log
...
EXP_TAG="${EXP_TAG:-phase8b_v8c_global_input_gate}"
...
echo "[launch] backbone=${BACKBONE_PATH} num_global_slots=${NUM_GLOBAL_SLOTS} global_slot_input_gate_only=True"
echo "[launch] ${CMD}"
