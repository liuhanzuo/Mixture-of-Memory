#!/usr/bin/env bash
# T2 chunked associative-recall (readout-pivot, 2026-06-14) — chunk_size=512.
...
# status/STAGING_REPORT_readout_pivot.md). chunk_size=512 -> n_ctx=7 (gap 3584).
# NOTE: eval must use the SAME --chunk_size 512 to match memory write granularity.
set -euo pipefail
...
RUN="T2_recall_chunk512_MASS_coef2_N128_s1234"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29960 \
  ...
  --chunk_size 512 --batch_size 2 --num_slots 128 --top_k 16 --selector_dim 128 \
  ...
  --t2_recall_mix_fraction 0.5 --t2_num_keys 1 --t2_gap_tokens 3584 \
  --use_readout_mass_bias --readout_mass_coef 2.0 \
