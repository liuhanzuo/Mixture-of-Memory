#!/usr/bin/env bash
# Per-doc CPT arm2 (remote H20-2, 28.59.80.196): chunk_size=256, intra-document chunk groups.
# Byte-identical to arm1 launch_mem_space_perdoc_chunk128.sh EXCEPT:
#   - chunk_size 128 -> 256 (this is the arm difference being ablated)
#   - remote node: conda torch-base python + WANDB_MODE=offline (remote can't reach wandb.ai)
#   - master_port 29780 -> 29782 (avoid clash)
# n_ctx fixed at 3 via curriculum 0:3 -> group_len = (3+1)*256 = 1024 tokens, all from
# the SAME document (context=3 chunks, target=1 chunk, adjacent in-doc).
# Data = MemLong/data/processed/dolmino_per_doc/train (shared share_303098609 FS, visible on .196).
...
RUN="mem_space_perdoc_chunk256_remote"
