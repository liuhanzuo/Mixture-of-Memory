#!/bin/bash
# Experiment H12: Architecture fix only (no contrastive)
#
# Tests if the cross_attn_modules fix alone helps with NIAH retrieval.
# H6/H6b/H7 never built cross_attn_modules — this is the first experiment
# with the architecture actually working. Same config as H7 but with the fix.
#
# contrastive_weight=0.0 (same as H7) — tests architecture fix alone.
...
LOGDIR=logs
mkdir -p $LOGDIR
LOGFILE=$LOGDIR/experiment_h12_arch_only_$(date +%Y%m%d_%H%M).log

echo "============================================"
echo "Experiment H12: Arch fix only (no contrastive)"
echo "  Same as H7 but with cross_attn_modules built"
echo "  NIAH: lambda=0.1, mix=0.10, contrastive=0"
echo "Output: outputs/experiment_h12_arch_only"
echo "Log:    $LOGFILE"
echo "============================================"
...
torchrun \
    ...
    --output_dir /apdcephfs_wzc1/share_303098609/pighzliu_code/Mixture-of-Memory/outputs/experiment_h12_arch_only \
