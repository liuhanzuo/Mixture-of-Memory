#!/usr/bin/env bash
# Self-study distillation on PG19 real long books (2026-06-16) — chunk_size=512, n_ctx=15.
# Derived from launch_distill_pg19_chunk512_nctx7.sh. Doubles the training window:
# group_len=(15+1)*512=8192 token (vs n_ctx=7 -> 4096), to bring the student's
# training context closer to the 32k inference length and try to break the 32k
# qa5 ceiling (n_ctx=7 broke 16k: 16 vs 13, but 32k stayed at 9=9; root cause =
# 32k inference is 8x the 4096 training window). PG19 single books are 60k-100k+
# tokens so they fully support the 8192-token window. PURE pg19 distillation arm
# (NO mass bias). Distill:
#   A: bidirectional KL (lambda=0.6) on teacher top-64 logits, and
#   B: 1-cos hidden matching on decoder layers 12,20,28,
# both read from the OFFLINE teacher cache distill_cache/pg19_512_nctx15.
#
# === USAGE ===================================================================
# Teacher cache built on .249 (diskB): distill_cache/pg19_512_nctx15
#   meta.json: n_ctx=15, chunk_size=512, distill_layers=[12,20,28], topk=64,
#   group_len=8192. Cache keys MUST match (--chunk_size 512 / --curriculum 0:15 /
...
#   setsid bash scripts/launch_distill_pg19_chunk512_nctx15.sh
# ==============================================================================
set -euo pipefail
...
RUN="distill_pg19_chunk512_nctx15_seed1234"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29981 \
  ...
  --last_chunk_loss_only --curriculum 0:15 --bptt_window 2 --no_detach_slots_in_selector \
  ...
  --distill_weight 1.0 --distill_hidden_beta 1.0 --distill_cache_dir distill_cache/pg19_512_nctx15 \
