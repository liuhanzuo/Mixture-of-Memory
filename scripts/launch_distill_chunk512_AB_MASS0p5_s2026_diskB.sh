#!/usr/bin/env bash
# Self-study distillation (v21) — chunk512 PURE dolmino arm, weak mass coef0.5.
# THIRD seed (2026) to confirm the "弱mass(coef0.5)+蒸馏=长程最佳" hypothesis.
# seed42 ran on B200/diskA, seed1234 on .249/diskB; this is seed2026 on .76/diskB.
# Reuses the SAME diskB teacher cache distill_cache/512 (shared FS with .249).
# Identical to launch_distill_chunk512_AB_MASS0p5_s1234_diskB.sh except seed + RUN.
set -euo pipefail
...
RUN="distill_chunk512_AB_MASS0p5_s2026_dolmino"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29972 \
