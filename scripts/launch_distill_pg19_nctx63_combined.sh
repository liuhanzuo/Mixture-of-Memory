#!/usr/bin/env bash
# Self-study distillation on PG19 (chunk_size=512, n_ctx=63) — nctx63 SOTA config
# + MECH arm: --use_shared_addressing --use_learnable_mass_bias (gp-8, commit d71afaa).
#
# Mechanism: the read path scores REAL slots with the SAME write-selector key geometry
# (normalize(q_proj_addr(hidden)) · normalize(K_sel(slots)+slot_key_bias)) instead of the
# independent q_proj·k_proj(slot->hidden) geometry. Targets the ~84% dead-slot read mass
# root cause (write/read addressing geometry gap). Smoke: OFF byte-identical; ON drops
# dead-slot read mass 0.868 -> 0.125 with grad-bearing routing key. Parallel arm to
# learnmassbias (which adds a learnable per-head written-ness read bias instead).
#
# Run on diskA (.196). Final judge = step250/500 BABILong W0 vs nctx63 SOTA
# (qa1 4k/8k/16k/32k = 23/12/14/11).
set -euo pipefail
...
RUN="distill_pg19_nctx63_combined"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29995 \
  ...
  --use_memory_xattn --memory_xattn_gate_init 0.4 --use_shared_addressing --use_learnable_mass_bias \
