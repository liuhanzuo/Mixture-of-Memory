  ...
  while true; do
    ...
    if [ "$kind" = "longeval" ]; then
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_longeval_mem_space.py \
        ...
        --output_name longeval_learnmass_s250_swa6 --lengths "$L" --num_samples 100 \
        --chunk_size 512 --swa_eval_chunks 6 \
        >>logs/eval_learnmass_longeval_ruler_swa6_gpu${gpu}.log 2>&1
    else
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_ruler_mem_space.py --model_type mem_space \
        --model_path "$MODEL" --checkpoint "$CKPT" --adapter_config "$ACFG" \
        --output_name ruler_learnmass_s250_swa6 --chunk_size 512 --swa_eval_chunks 6 \
...
wait
echo "LEARNMASS_LONGEVAL_RULER_SWA6_DONE"
