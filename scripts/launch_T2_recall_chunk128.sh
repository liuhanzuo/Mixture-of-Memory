...
# status/STAGING_REPORT_readout_pivot.md). chunk_size=128 -> n_ctx=28 (gap 3584).
# NOTE: eval must use the SAME --chunk_size 128 to match memory write granularity.
set -euo pipefail
...
RUN="T2_recall_chunk128_N128"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29932 \
  ...
  --chunk_size 128 --batch_size 2 --num_slots 128 --top_k 16 --selector_dim 128 \
