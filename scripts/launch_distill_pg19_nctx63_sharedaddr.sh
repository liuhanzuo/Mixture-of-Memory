...
# + MECH arm: --use_shared_addressing (gp-8, commit d71afaa).
#
...
RUN="distill_pg19_nctx63_sharedaddr"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29993 \
  ...
  --use_memory_xattn --memory_xattn_gate_init 0.4 --use_shared_addressing \
