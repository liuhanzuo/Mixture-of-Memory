    ...
    for task in tqdm(tasks, desc="tasks"):
        ...
        for length in tqdm(args.lengths, desc="lengths", leave=False):
            ...
            for i in tqdm(range(args.limit), desc=f"{task}/{length}", leave=False):
                ...
                # Qwen3 no-thinking control while preserving raw RULER completion prompt.
                # Mirrors apply_chat_template(enable_thinking=False) empty-think prefill.
                if task == "variable_tracking":
                    prompt = prompt + "<think>\n\n</think>\n\n"
                ...
                bare_q = _bare_question(prompt_for_query)
