#!/usr/bin/env bash
# T2 curriculum recall — chunk_size=1024, PROGRESSIVE seq length 4K->8K->16K->32K.
...
# curriculum n_ctx = 4->8->16->32 (x chunk_size 1024 = 4K/8K/16K/32K effective seq).
# Both dolmino background AND the T2 recall needle distance now follow n_ctx
# (NIAHChunkedDataset.set_n_ctx, commit 2c22d80).
# Equal data per stage: 16000 samples/stage (= the grid's single-length budget).
#   bs8 -> 128 samples/step -> 125 steps/stage -> 500 steps total.
#   save_interval=125 -> a checkpoint at the END of each stage (4K@125, 8K@250,
#   16K@375, 32K@500) so we can eval the curriculum trajectory stage by stage.
...
RUN="T2_recall_chunk1024_CURRICTEST_single0to4_B200_N128"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29952 \
  scripts/train_mem_space_dolmino_cpt.py \
  --model_path models/Meta-Llama-3-8B \
  --per_doc_data --dolmino_path MemLong/data/processed/dolmino_per_doc/train \
  --output_dir outputs/$RUN --total_steps 125 --lr 1e-4 --warmup_steps 12 \
  --chunk_size 1024 --batch_size 8 --num_slots 128 --top_k 16 --selector_dim 128 \
  ...
  --last_chunk_loss_only --curriculum 0:4 --bptt_window 2 \
  ...
  --save_interval 999999 --eval_interval 0 --eval_samples 30 --log_interval 5 \
