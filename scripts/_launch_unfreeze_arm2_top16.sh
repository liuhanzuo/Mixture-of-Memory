#!/bin/bash
# UNFREEZE-SWEEP arm2: unfreeze contiguous TOP-16 decoder layers (16..31).
# Only variable vs arm1 (=MVP, sparse {16,28,29,30,31}) is the unfreeze set.
# Everything else byte-identical to scripts/_launch_dirc_qa5sft_mvp.sh.
# Red lines: babilong_mix_fraction 0 (all-synthetic, never reads babilong test);
# pg19 background; warm-start from A-model step2000 (NOT a leaked ckpt).
set -euo pipefail
...
echo "[launch] $RUN (unfreeze TOP-16 layers 16..31, DDP, from step2000, lr3e-5, 400steps, mix=0)"
