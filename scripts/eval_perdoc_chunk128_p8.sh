#!/usr/bin/env bash
# Offline BABILong eval for per-doc chunk128 P8 arm (dedicated memory cross-attn read path, --use_memory_xattn gate_init 0.4).
# Mirror of eval_perdoc_chunk128_p7p9.sh. P8 = P7P9 knobs + isolated xattn read path.
# Gate: does P8 dedicated read path beat P7P9 qa5 (0k-32k=53/45/36/31/27/24/2) and cross >=4k noise floor more cleanly?
...
CKPT_DIR=outputs/mem_space_perdoc_chunk128_p8
ADAPTER_CONFIG=${CKPT_DIR}/adapter_config.json
CKPT=${CKPT_DIR}/mem_space_adapter.pt
RESULTS=babilong_results/perdoc_chunk128_p8
TASKS="qa1 qa2 qa5"
LOGDIR=logs/eval_perdoc_chunk128_p8
...
for L in "${LENGTHS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=$GPU setsid bash -c "$PYBIN scripts/run_babilong_mem_space.py \
    ...
    --results_folder $RESULTS --output_name perdoc_chunk128_p8_${L} \
