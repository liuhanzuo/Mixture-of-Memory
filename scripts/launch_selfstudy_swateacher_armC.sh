#!/usr/bin/env bash
# Self-study Arm C (2026-06-21):
#   teacher = offline frozen full-context cache (distill_cache/pg19_512_nctx15)
#   student = memory + 2 chunks train-side SWA (--swa_train_chunks 2)
set -euo pipefail
...
RUN="${RUN:-mem_space_selfstudy_armC_fullteacher_swa2_chunk512}"
NPROC="${NPROC:-8}"
...
setsid bash -c "CUDA_VISIBLE_DEVICES=$GPUS $PYBIN -m torch.distributed.run --nproc_per_node=$NPROC --master_port=$MASTER_PORT \
  ...
  --last_chunk_loss_only --swa_train_chunks 2 --curriculum $CURRICULUM --bptt_window 2 --no_detach_slots_in_selector \
