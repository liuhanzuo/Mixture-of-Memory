#!/usr/bin/env bash
# Phase-8B v7 — hybrid EMA + global always-on slots on Llama-3-8B-Instruct.
#
# Purpose: Compare directly with MemoryLLM-8B.
# Same v7 architecture as 1B version but with 8B backbone.
# Key: --num_global_slots 8, --gradient_checkpointing (required for 8B on 183 GiB B200)
#
# Launch (on B200 28.89.17.144 via wzc1 shared FS):
#   bash scripts/launch_phase8b_v7_global_slots.sh
#
# Target node: B200 28.89.17.144 (8× L20A, 183 GiB)
# Outputs: outputs/babilong_sft_phase8b_v7_global_slots_<timestamp>/
# Log:     logs/phase8b_v7_global_slots_<timestamp>.log
...
EXP_TAG="${EXP_TAG:-phase8b_v7_global_slots}"
...
echo "[launch] backbone=${BACKBONE_PATH} num_global_slots=${NUM_GLOBAL_SLOTS}"
echo "[launch] ${CMD}"
