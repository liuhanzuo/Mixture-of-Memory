#!/usr/bin/env bash
# 方案B: FIFO hidden-state memory — H2外推 arm, buffer=5 (=2560 tok), 最激进小buffer
# H2假说: FIFO buffer越小, BABILong长档(8k-32k)越好 (dilution假说)
# 与 b25/b50/b100 同口径(babilong_mix=0.15 default), 可直接对比 buffer 效应
...
RUN="mem_space_fifo_b5_chunk512"
mkdir -p logs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29804 \
