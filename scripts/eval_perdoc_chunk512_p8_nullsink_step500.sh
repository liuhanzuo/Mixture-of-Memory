#!/usr/bin/env bash
# Offline BABILong eval for per-doc chunk512 P8-NULLSINK arm, checkpoint step500.
...
# step500 already scored). Uses local idle GPU 0,1,2,3,5 (chunk128 training done;
...
RESULTS=babilong_results/perdoc_chunk512_p8_nullsink_step500
TASKS="qa1 qa2 qa5"
LOGDIR=logs/eval_perdoc_chunk512_p8_nullsink_step500
...
for L in "${LENGTHS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=$G setsid bash -c "$PYBIN scripts/run_babilong_mem_space.py \
    ...
    --results_folder $RESULTS --output_name perdoc_chunk512_p8_nullsink_step500_${L} \
