...
LR=${LR:-2e-5}
DATASET=${DATASET:-""}              # HuggingFace 数据集名称 (如 "msc")，空则使用合成数据
DATASET_MAX_SAMPLES=${DATASET_MAX_SAMPLES:-2000}  # 从 HF 数据集转换的最大样本数
MODEL=${MODEL:-""}                  # 自定义模型路径 (如 "../models/NousResearch--Llama-2-7b-hf")，空则使用默认
MODEL_DTYPE=${MODEL_DTYPE:-"bfloat16"}  # 模型精度: float16, bfloat16, float32
BACKBONE_TYPE=${BACKBONE_TYPE:-"swa"}   # backbone 类型: swa, full_attention
...
# ---- 模型路径解析 ---- #
# 如果用户指定了 MODEL 环境变量，自动更新 model yaml
if [[ -n "${MODEL}" ]]; then
    # 解析为绝对路径
    if [[ "${MODEL}" = /* ]]; then
        MODEL_PATH="${MODEL}"
    else
        MODEL_PATH="$(cd "${PROJECT_ROOT}" && realpath "${MODEL}" 2>/dev/null || echo "${PROJECT_ROOT}/${MODEL}")"
    fi
    
    # 自动检测模型的 hidden_size 和 num_heads
    _detect_model_arch() {
        python3 -c "
import json, sys
...
print(f'{hidden} {heads} {head_dim}')
" 2>/dev/null || echo '2048 16 128'
    }
    read _HIDDEN_SIZE _NUM_HEADS _HEAD_DIM <<< "$(_detect_model_arch)"
    
    info "检测到自定义模型: ${MODEL_PATH}"
    info "  hidden_size=${_HIDDEN_SIZE}, num_heads=${_NUM_HEADS}, head_dim=${_HEAD_DIM}"
    
    # 生成/更新 model yaml 配置
    _update_model_yaml() {
        local yaml_file="$1"
        local backbone_type="$2"
        cat > "${yaml_file}" <<YAML_EOF
# 自动生成的模型配置 (由 MODEL=${MODEL} 触发)
# 生成时间: $(date -Iseconds)
...
YAML_EOF
    ...
    _update_model_yaml "${PROJECT_ROOT}/configs/model/swa_qwen.yaml" "swa"
    _update_model_yaml "${PROJECT_ROOT}/configs/model/fullattn_qwen.yaml" "full_attention"
    ok "已自动更新 model yaml 配置"
else
    MODEL_PATH="$(cd "${PROJECT_ROOT}/../models/Qwen--Qwen3-1.7b" 2>/dev/null && pwd || echo "${PROJECT_ROOT}/../models/Qwen--Qwen3-1.7b")"
fi
...
run_phase3() {
    ...
    DATASET_ARGS=""
    if [[ -n "${DATASET}" ]]; then
        info "3.1 训练 L2 聚合器 (LoRA, ${TRAIN_EPOCHS} epochs, 数据集: ${DATASET})..."
        DATASET_ARGS="--dataset ${DATASET} --max-samples ${DATASET_MAX_SAMPLES}"
    else
        info "3.1 训练 L2 聚合器 (LoRA, ${TRAIN_EPOCHS} epochs, 合成数据)..."
    fi
    ...
    # 3.2 训练 L3 总结器
    if [[ -n "${DATASET}" ]]; then
        info "3.2 训练 L3 总结器 (LoRA, ${TRAIN_EPOCHS} epochs, 数据集: ${DATASET})..."
    else
        info "3.2 训练 L3 总结器 (LoRA, ${TRAIN_EPOCHS} epochs, 合成数据)..."
    fi
