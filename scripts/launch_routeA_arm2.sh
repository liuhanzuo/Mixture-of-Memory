#!/usr/bin/env bash
# ROUTE-A balance sweep arm2: identical to launch_mem_space_p11_chunk512_remote196.sh
# EXCEPT --entropy_aux_weight 0.0 -> 0.01 (+ total_steps 1000 for the sweep).
# Goal: lift usage_cov (~0.25, only ~32/128 slots used) via routing entropy regularizer.
set -euo pipefail
...
RUN="routeA_arm2_entropy001"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29811 \
  ...
  --selector_temperature 40 --load_balance_weight 0.0 --entropy_aux_weight 0.01 \
