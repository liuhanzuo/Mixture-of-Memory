#!/usr/bin/env bash
# n_ctx=63 PG19 distillation arm with only 64 memory slots and top_k=16.
# Purpose: compare against the standard 128-slot/top16 nctx63 SOTA to test whether
# explicit selection over 128 slots is helping or hurting relative to dense use of
# a small 16-slot bank.
set -euo pipefail
...
RUN="distill_pg19_nctx63_N64_top16"
mkdir -p logs outputs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29992 \
  ...
  --chunk_size 512 --batch_size 1 --num_slots 64 --top_k 16 --selector_dim 128 \
