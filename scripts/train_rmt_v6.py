"""RMT v6 training — aligned with official RMT paper.

Changes from v5:
1. CE-only loss (removed reconstruction loss entirely)
2. Z-forcing as probability-based teacher forcing instead of a loss term
3. Memory token LR = 10× base_lr (2e-4 vs 2e-5)
4. 20 epochs default (was 3)
5. Memory tokens initialized from backbone embedding layer
6. Dataset loops to support 20 epochs over small data
"""
...
def get_zforce_prob(epoch: int) -> float:
    """Probability of teacher-forcing ground-truth memory into a segment.
    Schedule: p(z) = 0.5 for epochs 1-5, 0.25 for epochs 6-10, 0.0 for epochs 11+.
    """
    if epoch < 5:
        return 0.5
    elif epoch < 10:
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/rmt_v6")
    ...
    parser.add_argument("--resume", type=str, default=None, help="Path to checkpoint directory for resuming (e.g. outputs/.../ckpt_step500/)")
    parser.add_argument("--resume_epoch", type=int, default=0, help="Epoch number to resume from (must match checkpoint)")
    ...
    if args.resume:
        if rank == 0:
            print(f"[RMT v6] Resuming from checkpoint: {args.resume} (epoch {args.resume_epoch})")
    ...
    rmt_memory = RMTMemory(
        ...
        use_reconstruction=False,  # v6: no reconstruction
    )
    rmt_memory = rmt_memory.to(device=device, dtype=torch.bfloat16)

    # Initialize memory tokens from backbone embedding layer (skip if resuming)
    if args.resume:
        rmt_path = os.path.join(args.resume, "rmt_memory.pt")
        rmt_memory.load_state_dict(torch.load(rmt_path, map_location=device))
        if rank == 0:
            print(f"[RMT v6] Loaded RMT memory from {rmt_path}")
    else:
        with torch.no_grad():
            base_model = model.get_base_model()
            embed_weight = base_model.model.get_input_embeddings().weight.data  # [vocab, hidden_dim]
            vocab_size = embed_weight.shape[0]
            # Sample random token indices and use their embeddings
            sampled_indices = torch.randint(0, vocab_size, rmt_memory.memory_embeddings.shape)
            rmt_memory.memory_embeddings.copy_(embed_weight[sampled_indices])

    if ddp:
        model = DDP(model, device_ids=[rank], find_unused_parameters=True)
        rmt_memory = DDP(rmt_memory, device_ids=[rank])

    # Optimizer: memory LR = 10× base LR
    optimizer = AdamW(
        [
    ...
    # Training loop
    if rank == 0:
        print(f"[RMT v6] Starting training: {len(dataloader)} steps/epoch, {args.num_epochs} epochs")
    ...
    while epoch < args.num_epochs:
        ...
        # Z-forcing probability for this epoch
        zforce_p = get_zforce_prob(epoch)
        ...
        for batch_idx, (input_ids, attn_mask) in enumerate(dataloader):
            ...
            old_memory = None
            prev_memory = None  # for Z-forcing teacher memory
            ...
            for seg_idx in range(num_segments):
                ...
                if old_memory is None:
                    ...
                    # Z-forcing: with probability p(z), replace with teacher-forced memory
                    if prev_memory is not None and torch.rand(1).item() < zforce_p:
                        mem = prev_memory.detach()
                    else:
                        mem = old_memory
                ...
                # Extract memory for next segment (teacher-forced ground truth)
                with torch.no_grad():
            ...
            # CE-only loss (no reconstruction, no Z-forcing prediction loss)
            ...
            if (batch_idx + 1) % args.grad_accumulation_steps == 0:
                ...
                if rank == 0 and step % args.log_every == 0:
                    ...
                    print(f"[RMT v6] Epoch {epoch} [{batch_idx+1}/{len(dataloader)}] "
                          f"loss={avg_loss:.4f} lr={lr:.2e} zforce_p={zforce_p:.2f} ETA={eta:.1f}h",
                          file=log_file)
                    hb = {
                        "timestamp": datetime.datetime.now().isoformat(),
                        "status": "running",
                        "progress": step / total_steps,
                        "metrics": {"loss": avg_loss, "learning_rate": lr, "epoch": epoch, "step": step, "zforce_p": zforce_p},
                        "extra": {},
        ...
        if rank == 0:
            elapsed = time.time() - t0
            print(f"[RMT v6] Epoch {epoch} done, elapsed: {elapsed/3600:.1f}h", file=log_file)
    ...
    if rank == 0:
        ...
        print("[RMT v6] Merging LoRA adapter into base model before save...")
        ...
        json.dump(hb, open(os.path.join(output_dir, "heartbeat.json"), "w"))
        print(f"[RMT v6] Training complete!", file=log_file)
        log_file.close()
