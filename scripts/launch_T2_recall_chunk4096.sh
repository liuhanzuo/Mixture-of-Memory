#!/usr/bin/env bash
# T2 chunked associative-recall (readout-pivot, 2026-06-14) — chunk_size=4096.
# Derived from launch_HARDOBJ_lastchunk_N128.sh. Adds a 50% mix of T2 synthetic
# associative-recall samples (NIAHChunkedDataset): a name->code needle is written
# (no_grad) into a context chunk; LM loss falls ONLY on the answer digits of the
# target/question chunk -> forces PRECISE memory readout (the bottleneck per
# status/STAGING_REPORT_readout_pivot.md). chunk_size=4096 -> n_ctx=7 (gap 28672).
# NOTE: eval must use the SAME --chunk_size 4096 to match memory write granularity.
set -euo pipefail
...
# chunk4096 over ~32k seq: first training step forward+backward can exceed the
# NCCL monitoring-heartbeat watchdog (fixed 480s, separate from the 2h collective
# timeout) -> false-positive SIGABRT before step 1 (observed 2026-06-15 01:46).
# Disable the heartbeat monitor + raise its timeout so the slow first step is OK.
export TORCH_NCCL_ENABLE_MONITORING=0
export TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC=1800
PYBIN="${PYTHON_BIN:-/opt/conda/envs/torch-base/bin/python}"
RUN="T2_recall_chunk4096_N128"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29935 \
  ...
  --chunk_size 4096 --batch_size 1 --num_slots 128 --top_k 16 --selector_dim 128 \
  ...
  --t2_recall_mix_fraction 0.5 --t2_num_keys 1 --t2_gap_tokens 28672 \
