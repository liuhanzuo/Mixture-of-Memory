...
def generate_with_mem_space(
    ...
    if readerattn_token or bm25_token:
    ...
    try:
        ...
        for step in range(max_new_tokens):
            if step == 0:
                # Prefill: full context, build KV cache.
                outputs = model(input_ids=cur, use_cache=True)
                past_kv = outputs.past_key_values
                logits = outputs.logits[:, -1, :]  # [1, vocab_size]
            else:
                # Decode: only the new token, reuse KV cache.
            ...
            # cur is still updated for compatibility (not used in decode steps).
