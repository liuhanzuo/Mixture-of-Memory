#!/usr/bin/env bash
# EXP-W1cont3k BABILong eval — W1(top_k32) continued from step1000 to step3000.
# Evaluates step2000 + step2500 + step3000(final) to trace whether longer training
# pushes 8k-32k long-range from W1 step1000 (qa5 42/42/41) toward base (48/45/44).
#
# LPT-balanced over 8 GPUs with mandatory per-GPU STAGGER (sleep G*25s) to avoid
# the known concurrent-load SIGKILL (8 jobs torch.load 10GB ckpt + 16GB model at
# t=0 -> OOM). Heavy lengths sample-sharded (32k->4, 16k->2) for even balance.
#
# Runs on LOCAL 8xH20 (disk-A, share_303098609) with project .venv python.
set -uo pipefail
...
CKPT_DIR="outputs/expW1_topk32_cont3k"
...
# Three checkpoints: step2000, step2500, step3000(final)
CK_NAME=(expW1_topk32_cont3k_step2000 expW1_topk32_cont3k_step2500 expW1_topk32_cont3k_step3000)
CK_FILE=("$CKPT_DIR/mem_space_adapter_step002000.pt" "$CKPT_DIR/mem_space_adapter_step002500.pt" "$CKPT_DIR/mem_space_adapter.pt")

LOGDIR="logs/eval_expW1cont3k"
mkdir -p "$LOGDIR"
...
# Build flat work items across ALL ckpts: (ckidx,task,length,shard_index,num_shards,weight)
...
NItems=$ni
echo "[$(date)] $NItems work items (3 ckpts)"
...
wait
echo "[$(date)] ALL_EVAL_DONE -> babilong_results/expW1_topk32_cont3k_step{2000,2500,3000}"
