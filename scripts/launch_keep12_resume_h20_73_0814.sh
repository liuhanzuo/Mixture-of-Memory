#!/bin/bash
# Launch keep12+fresh2 resume on .73 (8xH20, zwfy6 disk)
...
# Uses zwfy6 15.5M corpus (/dev/shm/dolmino_now15b.npy on .73; 126.9 GB verified)
# Resumes from the DISCOVERED newest keep12 ckpt (see preflight); asserts >= step166000.
...
LOG_FILE=logs/olmo2_7B_keep12fresh2_resume200k_73_0814.log

echo "=== keep12+fresh2 resume on .73 started at $(date) ===" | tee $LOG_FILE
...
NEWEST=$(ls -t $PROJECT_ROOT/outputs/olmo2_probe2_7B_keep12fresh2/step*.pt 2>/dev/null | head -1)
if [ -z "$NEWEST" ]; then
  echo "FATAL: no step*.pt in $PROJECT_ROOT/outputs/olmo2_probe2_7B_keep12fresh2" | tee -a $LOG_FILE; exit 1
fi
NEWSTEP=$(basename "$NEWEST" | tr -dc '0-9')
if [ "$NEWSTEP" -lt "166000" ]; then
  echo "FATAL: newest ckpt step$NEWSTEP < recorded 166000 -- refusing to lose steps" | tee -a $LOG_FILE; exit 1
fi
