#!/bin/bash
# Slot-forward continued pretrain: learnable slot embeddings + freeze backbone phase
# Phase 1 (steps 0-1000): Freeze base model, train slot_embeddings only (lr=base*100)
# Phase 2 (steps 1000+): Unfreeze all, joint training
...
LOGDIR=logs
mkdir -p $LOGDIR
LOGFILE=$LOGDIR/pretrain_slot_forward_$(date +%Y%m%d_%H%M).log
...
torchrun \
    ...
    --output_dir /apdcephfs_wzc1/share_303098609/pighzliu_code/Mixture-of-Memory/outputs/pretrain_slot_forward \
