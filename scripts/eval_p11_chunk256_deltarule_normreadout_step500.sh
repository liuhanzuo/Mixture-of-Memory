#!/usr/bin/env bash
# Offline BABILong eval for P11 chunk256 (delta_rule writeback + normalize_readout) ablation, step500.
# EXACT same protocol as scripts/eval_p8b_chunk256_topk8_step500.sh and the chunk256
# top_k16 baseline: run_babilong_mem_space.py, qa1/qa2/qa5, lengths 0k-32k, --limit 100,
# --chunk_size 256, bfloat16/sdpa. ONLY ckpt dir + results/output names + GPU list differ.
...
CKPT_DIR=outputs/mem_space_p11_chunk256_deltarule_normreadout
ADAPTER_CONFIG=${CKPT_DIR}/adapter_config.json
CKPT=${CKPT_DIR}/mem_space_adapter_step000500.pt
RESULTS=babilong_results/p11_chunk256_deltarule_normreadout_step500
TASKS="qa1 qa2 qa5"
LOGDIR=logs/eval_p11_chunk256_deltarule_normreadout_step500
...
run_on_gpu() {
  ...
  for L in "$@"; do
    ...
    CUDA_VISIBLE_DEVICES=$G $PYBIN scripts/run_babilong_mem_space.py \
      ...
      --results_folder $RESULTS --output_name p11_chunk256_deltarule_normreadout_step500_${L} \
