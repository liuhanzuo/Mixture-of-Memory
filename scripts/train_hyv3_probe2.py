#!/usr/bin/env python3
"""Probe #2 (minimal-architecture hypothesis) ported to the **Hy-MT2-30B-A3B**
(``hy_v3`` MoE, ~30B total / ~3B active).

Dedicated sibling of scripts/train_hunyuan_a13b_probe2.py (which handles the
larger ``hunyuan_v1_moe`` A13B). Kept as a SEPARATE file so the verified A13B path
is never touched. Same idea (Direction 4 / QCMem §3.1 "understand-then-generate
division of labour"): keep the FRONT ``keep_front_layers`` decoder layers of a
pretrained model, DROP the top layers, append ``n_fresh_layers`` FRESH decoder
layers to re-learn next-token prediction, and continue-train.

Base model: Hy-MT2-30B-A3B (Tencent Hunyuan, config model_type=hy_v3).
  * ~30B total / ~3B active, fine-grained MoE.
  * config: num_hidden_layers=48, hidden_size=2048, num_experts=128,
    num_experts_per_tok=8, moe_intermediate_size=768, intermediate_size=6912,
    num_shared_experts=1, GQA 32/4 heads, head_dim=128, qk_norm=True,
    initializer_range=0.006, vocab_size=120832, first_k_dense_replace=1
    (layer 0 dense HYV3MLP, layers 1..47 sparse HYV3MoE),
    **tie_word_embeddings=False** (lm_head is a real on-disk weight).

Differences vs the Hunyuan-A13B port (all handled below):
  1. **native transformers class** ``hy_v3`` (transformers >= 5.13.1). Use the
     ``.venv_hy3`` env. NO trust_remote_code, NO model_type patch.
  2. **head_dim already set** (128) in the config -> no attention crash patch
     needed (kept a defensive fallback anyway).
  3. **per-layer list field is ``mlp_layer_types``** (["dense", "sparse", ...],
     length = base num_hidden_layers). moe_topk / moe_intermediate_size /
     num_shared_experts are SCALARS here (unlike A13B's length-32 lists) so they
     need no truncation; we truncate ``mlp_layer_types`` to keep+fresh so the
     saved config is self-consistent (it also works untruncated since
     len==48 > 38, but truncating is cleaner).
  4. **tie_word_embeddings=False** -> lm_head.weight is an independent on-disk
     tensor. It is transplanted as a normal non-layer key (INHERITED, low LR).
     No untie step is required (the A13B FSDP tied-flat-param deadlock cannot
     occur); untie_output_embeddings() is still called but is a no-op here.
  5. **fresh MoE init std ~ initializer_range=0.006** (A13B used 0.02). The fresh
     sanity bounds are made RELATIVE to config.initializer_range.
  6. **30B is small enough for on-GPU FSDP** (no cpu_offload): on 8 cards each
     ~23.6B-param keep36+fresh2 shard holds fp32 master(~12GB)+AdamW(~24GB)+
     grad(~12GB) ~= 48GB/shard << 183GB. Pass --fsdp_cpu_offload 0 to get
     seconds/step instead of the A13B's 113s/step. NOTE the per-shard fp32
     master+optim scales with 1/world_size, so on ONLY 2 cards the keep36+fresh2
     config does NOT fit on-GPU (~189GB/shard) -> a 2-card smoke must either
     reduce keep_front (to fit) or enable --fsdp_cpu_offload 1.

Data / tokenizer -- IMPORTANT (embedding out-of-range trap):
  Hy-MT2 uses its own tokenizer (vocab 120832). The Hunyuan-A13B slimpajama .npy
  (max id 127960) and the Qwen3/Llama3 .npy files hold INCOMPATIBLE ids that
  index out of Hy-MT2's 120832-row embedding. You MUST train on a Hy-MT2-tokenized
  corpus: run scripts/preprocess_slimpajama.py --tokenizer <Hy-MT2 path>
  --chunk_size 2048 (Hy-MT2's tokenizer exposes eos_token_id=120025, so no
  --eos_token_id override is required). As a guard, we assert data.max() <
  vocab_size at startup.
...
most important anti-catastrophic-forgetting knob.
...
scripts/train_hunyuan_a13b_probe2.py or scripts/train_qwen3_arch_probe2.py.
"""
...
DEFAULT_MODEL_PATH = "/apdcephfs_wzc1/share_304376610/pighzliu_code/models/Hy-MT2-30B-A3B"
BASE_NUM_LAYERS = 48  # Hy-MT2-30B-A3B
...
def build_pruned_config(model_path, keep_front_layers, n_fresh_layers):
    """Load the Hy-MT2 (hy_v3) config, shrink it to keep+fresh layers, and truncate
    the per-layer ``mlp_layer_types`` list. Returns (cfg, total_layers)."""
    cfg = HYV3Config.from_pretrained(model_path, local_files_only=True)
    # native hy_v3 class -> keep model_type as-is (no trust_remote_code path).
    # head_dim is present in the Hy-MT2 config (128); defensive fallback only.
    ...
    # per-layer list field: mlp_layer_types (["dense"] + ["sparse"]*(L-1)).
    # It is derived from the base num_hidden_layers (len 48) at load time; truncate
    # to keep+fresh so the config stays self-consistent. moe_intermediate_size /
    # num_shared_experts / num_experts_per_tok are SCALARS here -> no truncation.
    v = getattr(cfg, "mlp_layer_types", None)
    if isinstance(v, (list, tuple)):
        assert len(v) >= total_layers, (
            f"mlp_layer_types len {len(v)} < keep+fresh {total_layers}")
        cfg.mlp_layer_types = list(v)[:total_layers]
        assert len(cfg.mlp_layer_types) == total_layers
    cfg.use_cache = False
...
def _copied_keys(state_dict, keep_front_layers):
    """Base-ckpt keys we transplant: all non-layer keys (embed / final-norm /
...
def _fresh_init_stats(model, keep_front_layers):
    """Fresh tail layers must retain proper hy_v3 init after transplant:
      * input_layernorm.weight all-ones (RMSNorm),
      * self_attn.q_proj.weight std ~ initializer_range,
      * (sparse layers only) mlp.experts.gate_up_proj std ~ initializer_range.
    Bounds are made RELATIVE to config.initializer_range (0.006 for Hy-MT2).
    Returns a dict; asserts on failure."""
    sd = model.state_dict()
    p = f"model.layers.{keep_front_layers}."
    init_range = float(getattr(model.config, "initializer_range", 0.02))
    lo, hi = 0.3 * init_range, 3.0 * init_range
    ...
    assert lo < q_std < hi, (
        f"fresh layer {keep_front_layers} q_proj std={q_std:.4f} not ~{init_range} "
        f"(bounds {lo:.4f}..{hi:.4f}) -> wrong init"
    )
    stats = {
    ...
    # fresh layer is sparse (MoE) iff mlp_layer_types[keep] == "sparse" (true for
    # keep_front>=1, since only layer 0 is dense). Guard the experts check.
    is_sparse = (p + "mlp.experts.gate_up_proj") in sd
    if is_sparse:
        gup_std = sd[p + "mlp.experts.gate_up_proj"].float().std().item()
        assert lo < gup_std < hi, (
            f"fresh layer {keep_front_layers} experts.gate_up_proj std={gup_std:.4f} "
            f"not ~{init_range} (bounds {lo:.4f}..{hi:.4f}) -> wrong MoE init"
        )
        stats["fresh_experts_gate_up_std"] = gup_std
    stats["fresh_layer_is_sparse"] = is_sparse
    return stats


def transplant_front(model, model_path, keep_front_layers, n_fresh_layers,
                     base_load_dtype, is_main):
    """Load the pretrained Hy-MT2 (native hy_v3 class) and transplant front
    keep_front layers + embed + final norm + lm_head into `model`. Fresh tail
    layers keep their hy_v3 random init. Runs the sanity asserts.
    ...
    full_cfg, _ = build_pruned_config(model_path, BASE_NUM_LAYERS, 0)  # full 48-layer cfg
    if is_main:
        logger.info(f"[transplant] loading full base ({base_load_dtype}) from {model_path} ...")
    base = HYV3ForCausalLM.from_pretrained(
    ...
    model.tie_weights()  # no-op (tie=False), kept for parity
    ...
    for mk in missing:
        ...
        if mk.startswith("model.layers.") and lid is not None:
            ...
            continue  # tied -> shares embed (not the case for Hy-MT2)
        else:
    ...
    # assert 3: copied-key count == (per-layer tensor count of a KEPT layer)*keep +
    # non-layer keys. NOTE layer 0 is dense (HYV3MLP) and layers 1.. are sparse
    # (HYV3MoE) with DIFFERENT per-layer tensor counts, so we sum per-kept-layer.
    per_layer_counts = {}
    for k in base_sd:
        lid = _layer_id(k)
        if k.startswith("model.layers.") and lid is not None:
            per_layer_counts[lid] = per_layer_counts.get(lid, 0) + 1
    n_nonlayer = len([k for k in base_sd if not k.startswith("model.layers.")])
    expected_copied = n_nonlayer + sum(per_layer_counts.get(i, 0)
                                       for i in range(keep_front_layers))
    assert len(keep_keys) == expected_copied, (
        f"[sanity3] copied {len(keep_keys)} != expected {expected_copied} "
        f"(={n_nonlayer} nonlayer + sum(front {keep_front_layers} layer tensors))"
    )
    ...
    if is_main:
        gup = fresh_stats.get("fresh_experts_gate_up_std")
        logger.info(
            ...
            f"left at hy_v3 init | max|model-base|={max_diff:.3e} (exact) | "
            f"fresh_ln_ones={fresh_stats['fresh_input_layernorm_all_ones']} "
            f"fresh_q_std={fresh_stats['fresh_q_proj_std']:.4f} "
            f"fresh_expert_std={gup if gup is None else f'{gup:.4f}'} "
            f"(init_range={fresh_stats['initializer_range']}) -> ALL CHECKS PASS"
        )
    return sanity


def build_hyv3_minimal(model_path, keep_front_layers, n_fresh_layers, dtype,
                       transplant=True, meta=False, is_main=True):
    """Build a (keep_front + n_fresh)-layer Hy-MT2 hy_v3 MoE model.
    ...
    transplant -> overwrite front keep layers + embed + norm + lm_head with the
                  pretrained weights and run sanity asserts (rank0 / single proc).
    ...
    if meta:
        with torch.device("meta"):
            model = HYV3ForCausalLM(cfg)
        return model, cfg, {"transplanted": False, "meta": True}

    model = HYV3ForCausalLM(cfg).to(dtype)
    if transplant:
...
def _classify_param(name, keep_front_layers, from_scratch):
    ...
    + norm + lm_head -> low LR). from_scratch -> everything 'fresh'.
    ...
    # embed_tokens / final norm / lm_head -> all pretrained -> inherited
    return "inherited"


def apply_freeze_front(model, keep_front_layers, is_main):
    """Arm A: freeze inherited front layers + embed + norm + lm_head; train fresh
    tail only."""
    n_frozen = n_train = 0
...
def build_param_groups(model, args, is_main):
    ...
    fresh (tail layers): base_lr=args.lr. inherited (front+embed+norm+lm_head):
    lr_inherited."""
    specs = {
...
# ---------------------------------------------------------------------------
# FSDP tied-weight guard (no-op for Hy-MT2, tie_word_embeddings=False)
# ---------------------------------------------------------------------------
def untie_output_embeddings(model, is_main):
    """Make lm_head.weight independent from embed if they are tied, and flip
    cfg.tie_word_embeddings=False. Prevents the FSDP first-forward all-gather
    size mismatch that a shared embed/lm_head flat-param causes across meta ranks
    (see the A13B script for the full diagnosis).

    Hy-MT2 has tie_word_embeddings=False so this returns immediately (no-op); kept
    for parity + robustness if a tied hy_v3 variant is ever passed in."""
    cfg = model.config
...
def wrap_fsdp(model, args, local_rank, is_main):
    ...
    auto_wrap = functools.partial(
        ...
        transformer_layer_cls={HYV3DecoderLayer},
    )
    ...
    if is_main:
        logger.info(f"[fsdp] FULL_SHARD wrapped on HYV3DecoderLayer; "
...
# ---------------------------------------------------------------------------
# dry-run structural validation (no GPUs, no 57GB base load)
# ---------------------------------------------------------------------------
def _dry_run(args):
    ...
    model, cfg, _ = build_hyv3_minimal(
    ...
    l_fresh = model.model.layers[args.keep_front_layers]  # a fresh (sparse) layer
    ...
    logger.info(f"[dry_run] REAL cfg -> layers={n_layers} params={n_params/1e9:.2f}B "
                f"head_dim={cfg.head_dim} fresh_mlp={type(l_fresh.mlp).__name__} "
                f"experts.gate_up_proj={tuple(l_fresh.mlp.experts.gate_up_proj.shape)} "
                f"experts.down_proj={tuple(l_fresh.mlp.experts.down_proj.shape)} "
                f"tie(lm_head is embed)={tied}")
    assert tied == bool(cfg.tie_word_embeddings)
    # per-shard on-GPU fp32 master+AdamW(2x)+grad estimate (16 bytes/param) vs cards
    for ws in (2, 8):
        gb = n_params * 16 / 1e9 / ws
        logger.info(f"[dry_run] est on-GPU fp32 master+AdamW+grad @ world_size={ws}: "
                    f"{gb:.1f} GB/shard (+ bf16 unshard + activations)")
    del model
    gc.collect()

    # (2) shrunk end-to-end CPU build: exercises real init + transplant + all
    #     asserts + fresh-MoE-init + param classification WITHOUT the 57GB base.
    ...
    small.num_experts_per_tok = 2
    small.moe_intermediate_size = 32
    small.num_shared_experts = 1
    ...
    pruned = HYV3ForCausalLM(small).to(torch.float32)
    base = HYV3ForCausalLM(small).to(torch.float32)   # stand-in "pretrained"
    base_sd = base.state_dict()
    ...
    logger.info(f"[dry_run] shrunk transplant OK: copied={len(keep_keys)} "
                ...
                f"fresh_expert_std={fresh.get('fresh_experts_gate_up_std')} "
                f"tie={tie}")
...
def main():
    ...
    p.add_argument("--data_path", type=str, default="",
                   help="Hy-MT2-tokenized (N, seq_len) uint32 npy. Required for training.")
    p.add_argument("--output_dir", type=str, default="")
    p.add_argument("--model_path", type=str, default=DEFAULT_MODEL_PATH)
    p.add_argument("--keep_front_layers", type=int, default=36)
    p.add_argument("--n_fresh_layers", type=int, default=2)
    p.add_argument("--freeze_front", action="store_true",
                   help="Arm A: freeze inherited (front+embed+norm+lm_head), train fresh tail only")
    ...
    p.add_argument("--lr_inherited", type=float, default=2e-5,
                   help="LR for inherited front layers + embed + norm + lm_head")
    ...
    p.add_argument("--fsdp_cpu_offload", type=int, default=0,
                   help="0 (DEFAULT for Hy-MT2-30B) = keep FSDP params+grads+optim ON-GPU. "
                        "On 8 cards the keep36+fresh2 shard is ~48GB (fp32 master+AdamW+grad) "
                        "<< 183GB -> seconds/step (vs the A13B 65B config's 113s/step under "
                        "offload). Set 1 ONLY when the per-shard fp32 master+optim would not "
                        "fit on-GPU: the per-shard cost scales 1/world_size, so keep36+fresh2 "
                        "does NOT fit on 2 cards (~189GB/shard) -> a 2-card smoke needs either "
                        "a smaller --keep_front_layers or --fsdp_cpu_offload 1. Offload keeps "
                        "only the active layer's bf16 unshard + activations on-GPU (fp32 master "
                        "on CPU pinned RAM, AdamW update on CPU); fp32 master logic is unchanged.")
    p.add_argument("--dry_run_build", action="store_true",
                   help="meta + shrunk-CPU structural/transplant-logic validation, then exit. "
                        "No GPUs, no 57GB base load.")
    ...
    if ddp:
        # NCCL init-timeout fix: rank0 materialises the transplant on CPU (slow:
        # 57GB base disk load + fresh-model init) BEFORE the first collective (the
        # FSDP sync_module_states broadcast). Two-part fix:
        #   (a) timeout=2h so the PG tolerates rank0's slow assembly;
        #   (b) device_id=... forces EAGER communicator formation inside
        #       init_process_group while all ranks are aligned -> no lazy-init race.
    ...
    if is_main:
        ...
        logger.info(f"=== Hy-MT2-30B (hy_v3) minimal-arch probe#2 [{arm}] ===")
        logger.info(f"world_size={world_size} bs={args.batch_size} "
                    ...
                    f"max_steps={args.max_steps} cpu_offload={bool(args.fsdp_cpu_offload)} "
                    f"fp32 master + FSDP(bf16 compute)")
    ...
    model, cfg, sanity = build_hyv3_minimal(
        args.model_path, args.keep_front_layers, args.n_fresh_layers,
    ...
    # ---- FSDP wrap ----
    if ddp:
        # tie=False for Hy-MT2 so untie is a no-op; kept for parity/robustness.
        untie_output_embeddings(model, is_main)
        model = wrap_fsdp(model, args, local_rank, is_main)
    else:
        model = model.to(device)

    # ---- data (Hy-MT2-tokenized; guard against embedding out-of-range) ----
    ...
    if is_main:
        ...
        assert vmax < cfg.vocab_size, (
            ...
            f"(embedding out-of-range). Re-tokenize slimpajama with the Hy-MT2 tokenizer.")
