...
# Resumes from step124500.pt (zwfy6, has optimizer state, faithful REMAPPED)
...
LOG_FILE=logs/olmo2_7B_keep8fresh2_resume200k_82.log
...
CKPT=$PROJECT_ROOT/outputs/olmo2_probe2_7B_keep8fresh2/step124500.pt
if [ ! -f "$CKPT" ]; then
...
torchrun \
  ...
  --keep_last_n 3 \
  --keep_milestones 8 \
  --keep_steps 83500,121000,124000,124500,150000,175000,200000 \
  --data_path $DATA_PATH \
