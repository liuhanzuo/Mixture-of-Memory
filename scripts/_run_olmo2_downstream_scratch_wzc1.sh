#!/usr/bin/env bash
# ============================================================================
# from_scratch (random-init 16L OLMo-2-7B, keep14/fresh2 shell) DOWNSTREAM MC
# capability eval — the paired capability read for the just-finished Paper B
# control arm `olmo2_probe2_7B_keep14fresh2_fromscratch` (step200000).
# WZC1 variant: PY=.venv (sm_100 B200/L20A); runs on .252.
# prepare_data (proxy, CPU) -> 8-GPU sharded MC -> merge.  Mirrors
# scripts/_run_olmo2_probe2_downstream_8gpu.sh (which hardcodes torch-base).
# ============================================================================
set -uo pipefail
...
$PY scripts/eval_olmo2_probe2_downstream.py --prepare_data --tasks "$TASKS" \
    >logs/olmo2_downstream_scratch_prep.log 2>&1
echo "[$(date '+%F %T')] prepare_data done rc=$?"

# (2) 8 shard MC procs — model is local; keep proxy for any dataset metadata refetch
...
for g in 0 1 2 3 4 5 6 7; do
  CUDA_VISIBLE_DEVICES=$g $PY scripts/eval_olmo2_probe2_downstream.py \
    ...
    --output_name "$NAME" \
    >logs/olmo2_downstream_scratch_shard${g}.log 2>&1 &
  PIDS+=($!)
...
$PY scripts/eval_olmo2_probe2_downstream.py --merge --output_name "$NAME" --tasks "$TASKS" \
    >logs/olmo2_downstream_scratch_merge.log 2>&1
cat "olmo2_downstream_results/$NAME/summary.json" 2>/dev/null | head -60
touch logs/olmo2_downstream_scratch_DONE
