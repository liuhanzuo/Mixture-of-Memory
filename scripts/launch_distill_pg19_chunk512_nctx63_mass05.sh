...
# + WEAK readout mass bias (coef=0.5). Parallel control arm to the PURE-distill
# launch_distill_pg19_chunk512_nctx63.sh. Motivation: the readout-attack result found
# weak-mass(coef~0.5)+distill was the long-range optimum (32k 6->9, inverted-U) at the
# 8k window; this arm tests whether that combo still wins at the scaled-up 32k window
# (group_len=(63+1)*512=32768). Identical to the pure arm except:
#   --use_readout_mass_bias --readout_mass_coef 0.5
# Shares the SAME offline teacher cache distill_cache/pg19_512_nctx63 (diskA FS).
# Run on diskA .196 (shares FS+cache with local; no rsync):
...
#   setsid bash scripts/launch_distill_pg19_chunk512_nctx63_mass05.sh
set -euo pipefail
...
RUN="distill_pg19_chunk512_nctx63_mass05"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29984 \
