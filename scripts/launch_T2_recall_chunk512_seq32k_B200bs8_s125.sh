#!/usr/bin/env bash
# T2 chunked associative-recall — chunk_size=512, seq 32k (gap 28672 -> ~56 ctx chunks).
# B200 variant: leverages 183GB/card to run a TRUE 32k long-sequence readout-attack run
# (the model has only ever seen ~2-4k, so 32k is the extrapolation regime we must test).
# chunk512 is the SAFE long-seq loader (chunk2048 first-step DDP-hangs). bs bumped to 2.
# NOTE: eval must use --chunk_size 512 to match memory write granularity.
set -euo pipefail
...
RUN="T2_recall_chunk512_seq32k_B200bs8s125_N128"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29948 \
  ...
  --chunk_size 512 --batch_size 8 --num_slots 128 --top_k 16 --selector_dim 128 \
