#!/usr/bin/env bash
# P11 ablation on P8 chunk512 base: delta-rule writeback + normalized readout.
# Identical to launch_mem_space_p8_nullsink_chunk512_diskB.sh EXCEPT:
...
RUN="expL1KEY_indepkey_N128"
# H20 batch tuning (2026-06-07): physical bs4 x grad_accum1 x 8gpu = eff_batch 32, held
# constant vs prior bs1 x grad_accum4. H20 chunk512 ceiling ~bs7 (86GiB); bs4=71GiB leaves
# 24GiB margin. Pure throughput win, identical optimization dynamics (bs2==2xbs1 verified rel<1e-4).
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29901 \
  ...
  --use_delta_rule_writeback --normalize_readout --independent_slot_key \
