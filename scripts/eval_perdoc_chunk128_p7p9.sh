#!/usr/bin/env bash
# Offline BABILong eval for per-doc chunk128 P7+P9 arm (route_aux=1.0 + LFB + num_global_slots=4).
# Mirror of eval_perdoc_chunk128_routeaux.sh (R3-2 recipe) for the P7P9 checkpoint.
# Gate: does P7+P9 cross noise floor at >=4k AND beat routeaux routing?
...
CKPT_DIR=outputs/mem_space_perdoc_chunk128_p7p9
ADAPTER_CONFIG=${CKPT_DIR}/adapter_config.json
CKPT=${CKPT_DIR}/mem_space_adapter.pt
RESULTS=babilong_results/perdoc_chunk128_p7p9
TASKS="qa1 qa2 qa5"
LOGDIR=logs/eval_perdoc_chunk128_p7p9
...
for L in "${LENGTHS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=$GPU setsid bash -c "$PYBIN scripts/run_babilong_mem_space.py \
    ...
    --results_folder $RESULTS --output_name perdoc_chunk128_p7p9_${L} \
