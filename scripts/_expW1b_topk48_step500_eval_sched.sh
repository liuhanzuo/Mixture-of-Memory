#!/usr/bin/env bash
# EXP-W1b top_k48 step500 BABILong eval — sharded LPT scheduler over 8 GPUs.
# 1 ckpt x 3 tasks x 7 lengths = 21 cells. Heavy lengths are sample-sharded
# (32k -> 4 shards/task, 16k -> 2 shards/task) so the 33 resulting work items
# LPT-balance evenly across 8 GPUs. Shards of the same (task,length) share one
# output_name folder (file suffix _shard{i}of{N}); score_nested_babilong.py
# globs+sums them transparently.
set -uo pipefail
...
CKPT_DIR="outputs/expW1b_topk48"
CKPT_FILE="$CKPT_DIR/mem_space_adapter_step000500.pt"
ADAPTER_CONFIG="$CKPT_DIR/adapter_config.json"
RESULTS="babilong_results/expW1b_topk48_step500"
LOGDIR="logs/eval_expW1b_topk48_step500"
CHUNK_SIZE=512
LIMIT=100
MAX_NEW_TOKENS=20
...
# num_shards per length (heavy -> more shards)
nshards_for() {
  case "$1" in
    32k) echo 4;; 16k) echo 2;; *) echo 1;;
  esac
}
# per-item weight = chunks(length)/num_shards (chunks ~ ctx/512)
chunks_for() {
  case "$1" in
...
# Build flat work items: (task,length,shard_index,num_shards,weight)
...
run_gpu_items() {
  ...
  for c in "${items[@]}"; do
    ...
    local out_name="expW1b_topk48_step500_${L}"
    local shard_tag=""
    [ "$ns" -gt 1 ] && shard_tag="_shard${si}of${ns}"
    local csv="$RESULTS/$out_name/${task}_${L}${SUFFIX_BASE}${shard_tag}.csv"
    # skip-complete: expected rows = samples in this shard + 1 header
    local exprows
