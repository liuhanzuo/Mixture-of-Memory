#!/usr/bin/env python3
"""Control experiment: fine-tune base Llama2-7B WITHOUT SparseMemory wrapper.

Same data, same hyperparameters, same steps as train_sparse_memory.py,
but using vanilla HuggingFace LlamaForCausalLM + DDP.

This isolates the contribution of SparseMemory vs plain fine-tuning.
"""
import os, sys, math, time, argparse, json, logging
...
def main():
    parser = argparse.ArgumentParser(description="Control: vanilla Llama fine-tuning")
    ...
    if is_main:
        logger.info(f"=== Control Experiment: Vanilla Llama Fine-tuning ===")
    ...
    # Tokenizer (only needed for saving)
    ...
    if is_main:
        ...
        n_gb = n_params * 2 / 1e9
        logger.info(f"Model size: {n_gb:.2f} GB params + {n_buffers * 2 / 1e6:.2f} MB buffers")
