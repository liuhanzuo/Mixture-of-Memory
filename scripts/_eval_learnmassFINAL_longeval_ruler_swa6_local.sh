#!/usr/bin/env bash
# learnmassbias FINAL ckpt LongEval + RULER W6 eval on local (8x H20, shared diskA pool).
# W6 window (high SWA). Complements B200 W0 + .174 W2 for FINAL-ckpt downstream window curve.
# Tests whether lever-2 long-train (no collapse) transfers to downstream LongEval/RULER.
# --chunk_size 512 --swa_eval_chunks 6
set -uo pipefail
...
CKPT="$ROOT/outputs/distill_pg19_nctx63_learnmassbias/mem_space_adapter.pt"
  ...
  while true; do
    ...
    if [ "$kind" = "longeval" ]; then
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_longeval_mem_space.py \
        ...
        --output_name longeval_learnmassFINAL_swa6 --lengths "$L" --num_samples 100 \
        --chunk_size 512 --swa_eval_chunks 6 \
        >>logs/eval_learnmassFINAL_longeval_ruler_swa6_local_gpu${gpu}.log 2>&1
    else
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_ruler_mem_space.py --model_type mem_space \
        --model_path "$MODEL" --checkpoint "$CKPT" --adapter_config "$ACFG" \
        --output_name ruler_learnmassFINAL_swa6 --chunk_size 512 --swa_eval_chunks 6 \
...
wait
echo "LEARNMASS_FINAL_LONGEVAL_RULER_SWA6_local_DONE"
