#!/usr/bin/env bash
# T2 chunked associative-recall — chunk_size=1024, seq 32k (gap 31744 -> n_ctx=31).
# Fills the chunk1024 x 32k cell of the chunk_size x seq_length grid (readout-pivot).
# chunk1024 is a safe loader (chunk2048 first-step DDP-hangs; 1024 does not).
...
RUN="T2_recall_chunk1024_seq32k_N128"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29944 \
  ...
  --t2_recall_mix_fraction 0.5 --t2_num_keys 1 --t2_gap_tokens 31744 \
