#!/usr/bin/env bash
# Per-slot raw-KV cache upper-bound test (2026-06-22).
# Based on launch_distill_pg19_chunk512_nctx63.sh, with --use_slot_kv_cache.
# Do not run automatically; launch manually when an 8-GPU node is free.
set -euo pipefail
...
RUN="slot_kv_cache_pg19_chunk512_nctx63"
mkdir -p logs outputs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29987 \
  ...
  --use_slot_kv_cache --slot_kv_cache_layer 16 \
