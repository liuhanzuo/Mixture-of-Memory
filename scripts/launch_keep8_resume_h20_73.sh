#!/bin/bash
# Launch keep8+fresh2 resume on .73 (8xH20, zwfy6 disk)
# Uses zwfy6 15.5M corpus (already in /dev/shm/dolmino_now15b.npy on .73)
# Resumes from step121000_full.pt (zwfy6, has optimizer state, faithful REMAPPED)
...
LOG_FILE=logs/olmo2_7B_keep8fresh2_resume200k_73.log

echo "=== keep8+fresh2 resume on .73 started at $(date) ===" | tee $LOG_FILE
...
CKPT=$PROJECT_ROOT/outputs/olmo2_probe2_7B_keep8fresh2/step121000_full.pt
if [ ! -f "$CKPT" ]; then
