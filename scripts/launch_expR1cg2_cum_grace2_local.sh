...
RUN="expR1cg2_cum_grace2"
# L20A chunk512: physical bs4 x grad_accum1 x 8gpu = eff_batch 32 (same as P11).
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29845 \
  ...
  --dead_slot_reset_interval 8 --dead_slot_grace_chunks 2 --dead_slot_reset_mode strided_current \
  --dead_slot_criterion cumulative \
