#!/usr/bin/env bash
# EXP-R1 (2026-06-11): two-stage dead-slot recycling on the P11 chunk512 base.
# Identical to launch_mem_space_p11_chunk512_remote196.sh EXCEPT it adds:
#   + --dead_slot_reset_interval 8   (reset never-selected slots every 8 chunks)
#   + --dead_slot_grace_chunks 1     (force-write recycled slots for 1 chunk)
#   + --dead_slot_reset_mode strided_current  (diverse strided current content)
#
# Goal: break the L1 cold-start "rich-get-richer" deadlock (gp-59 root-cause
# 20260611_dead_slot_recycling.md). ~91/128 slots stay frozen at their chunk-0
# token snapshot while ~32 live slots monopolise all content. EXP-R1 periodically
# refills never-selected slots with diverse current-chunk content (WRITE-SIDE
# only; the greedy selector is NEVER perturbed — the discriminator vs the
# rejected ROUTE-A arm4 read-side noise which collapsed top1_sim 0.99->0.11).
# Telemetry gate (EXP-D2, same commit): usage_cov UP (>0.5) AND top1_sim NOT
# collapsed (stays ~0.1-0.2) AND dead_slot_frac DOWN; judge qa1/qa5 8k-32k.
#
# ⚠️ DO NOT LAUNCH until a node is free (ckpt not urgent; team-lead decides
#    when to start). 32k≈63 chunks → ~7 reset events/sample at interval 8.
