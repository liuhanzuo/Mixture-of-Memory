...
export WANDB_MODE=offline OMP_NUM_THREADS=1 TOKENIZERS_PARALLELISM=false PYTHONPATH=$PWD PATH=/opt/conda/bin:$PATH
...
.venv_hy3/bin/python -m torch.distributed.run --standalone --nproc_per_node 8 \
  ...
  --keep_front_layers 24 --n_fresh_layers 2 --max_steps 2000 \
  --seq_len 2048 --batch_size 1 --grad_accumulation_steps 8 \
  --lr 1e-4 --lr_inherited 2e-5 --warmup_steps 150 \
  --log_every 10 --save_every 500 --gradient_checkpointing 1 \
