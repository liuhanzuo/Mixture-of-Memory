#!/usr/bin/env bash
# Phase-3 S5 passkey GATING — eval a trained S5 checkpoint (single_layer_mem=16)
# with the SAME protocol as S0/S2/S4b. IDENTICAL to eval_landmark_S2_passkey.sh
# EXCEPT REPO points at the ISOLATED S5 TREE so run_passkey imports the S5
# llama_mem (which honors config.single_layer_mem=16 saved in the ckpt). Using
# the S4b-tree llama_mem would silently IGNORE single_layer_mem -> run full-32
# grouped-softmax = wrong axis.
#   n_garbage 0/4000/8000/15000/30000/60000/115000 (~70/1.1k/2.2k/4k/8k/16k/32k tok)
#   50 tests/length, top_k=5, mem arm, 8-way GPU shard on 本机.
# Usage: CKPT=<ckpt-dir> CK_NAME=s5_step1000 bash scripts/eval_landmark_S5_passkey.sh
set -euo pipefail

PROJECT_ROOT="${PROJECT_ROOT:-/apdcephfs_zwfy6/share_303098609/pighzliu_code/Mixture-of-Memory}"
EXT="$PROJECT_ROOT/external"
REPO="$EXT/landmark_s5_tree/llama"          # <-- S5 isolated tree (single_layer_mem aware)
PY="$EXT/landmark_venv/bin/python"

CKPT="${CKPT:?set CKPT to the checkpoint dir}"
CK_NAME="${CK_NAME:-s5_ckpt}"
OUTDIR="$EXT/landmark/results_s2/$CK_NAME"
mkdir -p "$OUTDIR"
...
export LM_REPO="$REPO"          # force run_passkey to import the S5-tree llama_mem (single_layer_mem aware)
...
echo "launched 8 shards for $CK_NAME (S5 tree), pids: ${pids[*]}"
fail=0
...
for f in sorted(glob.glob(os.path.join(d, "mem_shard*of8.csv"))):
    with open(f) as fh:
        for r in csv.DictReader(fh):
            n = int(r["n_garbage"]); c = int(r["correct"]); t = int(r["num_tests"])
