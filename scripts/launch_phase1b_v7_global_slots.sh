#!/usr/bin/env bash
# Phase-1B v7 — hybrid EMA + global always-on slots on Llama-3.2-1B-Instruct.
#
# Experiment: most slots use EMA as usual; the LAST num_global_slots indices
# receive direct-replacement writes (slot ← s_new) on EVERY forward call,
# bypassing top-k routing. These "global" slots act as an EMA-free accumulation
# register, expected to improve counting/tracking (qa7-qa10) while keeping
# retrieval behaviour for the remaining regular slots.
#
# Key difference from v2: adds --num_global_slots 8
# Key difference from v6: only 8/512 slots use replacement; 504 still use EMA.
#
# Launch:  bash scripts/launch_phase1b_v7_global_slots.sh
# Outputs: outputs/babilong_sft_phase1b_v7_global_slots_<timestamp>/
# Log:     logs/phase1b_v7_global_slots_<timestamp>.log
...
EXP_TAG="${EXP_TAG:-phase1b_v7_global_slots}"
...
echo "[launch] num_global_slots=${NUM_GLOBAL_SLOTS} (slots $(( NUM_SLOTS - NUM_GLOBAL_SLOTS ))..$((NUM_SLOTS - 1)) always-on)"
echo "[launch] ${CMD}"
