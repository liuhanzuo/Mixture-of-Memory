...
DATA_PATH="data/mag_train_recall.jsonl"
...
LOG_EVERY=100
SAVE_EVERY=2000
OUTPUT_DIR="outputs/recall_bce_${TIMESTAMP}"

mkdir -p "$OUTPUT_DIR"

echo "============================================================"
echo "  🧠 Recall Data Training — ${NUM_GPUS} GPUs"
echo "============================================================"
echo "  模型:           $MODEL_PATH"
echo "  数据:           $DATA_PATH (50K recall 样本)"
echo "  输出:           $OUTPUT_DIR"
echo "  GPU:            $NUM_GPUS"
...
hb.update('init', progress=0.0, extra={'num_gpus': ${NUM_GPUS}, 'data': 'recall', 'loss': 'bce'})
...
if [ "$NUM_GPUS" -eq 1 ]; then
    ...
    torchrun --nproc_per_node=$NUM_GPUS --master_port=29502 scripts/train_mag.py $COMMON_ARGS 2>&1 | tee "$LOG_FILE"
fi
