#!/usr/bin/env bash
# Dense-write capacity matrix: top_k=num_slots=512 (global soft-write, no "never-updated" slot).
# High-capacity point matching the N512 DRoff control. bs1ga4 (high-N OOM guard).
# Runs on diskB node 28.49.57.76 (8x H20, share_304376610). PYBIN=.venv default.
set -euo pipefail
...
RUN="expDENSE_N512_topkAll"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29894 \
  ...
  --chunk_size 512 --batch_size 1 --num_slots 512 --top_k 512 --selector_dim 128 \
