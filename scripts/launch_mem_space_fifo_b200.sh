#!/usr/bin/env bash
# 方案B: FIFO hidden-state memory (MemoryLLM-style write, slot-free full-attn read)
# Runs on B200 .53 (28.88.184.53, 8× L20A 183GB, wzc1 disk, port 22)
# Key flags: --use_fifo_memory --fifo_buffer_chunks 50
# No slot routing / no delta-rule / no L3 — pure FIFO buffer baseline.
# 2026-06-24
set -euo pipefail
...
RUN="mem_space_fifo_b50_chunk1024"
mkdir -p logs
