#!/usr/bin/env bash
# 8-GPU sharded likelihood-MC DOWNSTREAM eval -- CORE 6-task, single config:
# the APEX prune-heal point OLMo-2-7B keep14+fresh2 (16L/32=50%), step128000
# (fully converged heal). EVAL-ONLY (forward-only) on .73. Companion / same
# harness as scripts/_run_olmo2_probe2_downstream_8gpu.sh (verbatim 8-shard
# [g::8] + merge; acc = sum(correct)/sum(n)).
set -u
...
DONE=logs/olmo2_downstream_keep14_DONE
rm -f "$DONE"

# config rows: "output_name|base_model|ckpt(empty=full base)"
CONFIGS=(
  "7B_keep14_step128000|../models/OLMo-2-1124-7B|outputs/olmo2_probe2_7B_keep14fresh2/step128000.pt"
)

echo "[$(date '+%F %T')] prepare_data (cache all tasks)"
$PY scripts/eval_olmo2_probe2_downstream.py --prepare_data --tasks "$TASKS" \
  > logs/olmo2_downstream_keep14_prepare.log 2>&1
tail -8 logs/olmo2_downstream_keep14_prepare.log
