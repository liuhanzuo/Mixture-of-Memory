#!/usr/bin/env bash
# P1 no-recon CONTROL (local H20-1): identical to recon arm but l_recon_weight=0.0.
# Pairs with scripts/launch_dolmino_recon_diskA.sh (recon=0.1) on remote 28.59.80.196.
# Relaunch after allreduce-desync fix (every rank issues identical all_reduce seq).
set -euo pipefail
...
RUN="dolmino_norecon_local_v2"
setsid bash -c "$PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29770 \
