#!/usr/bin/env bash
# 方案B: FIFO hidden-state memory — H2外推 arm, buffer=5 (=2560 tok), 最激进小buffer, 干净版(无泄漏)
# H2假说: FIFO buffer越小, BABILong长档(8k-32k)越好 (dilution假说)
# NOLEAK: babilong_mix_fraction=0, 排除 eval-split 数据泄漏, 测真实 buffer 效应
# Runs on H20 盘B (PROJECT_ROOT=/apdcephfs_zwfy6/share_304376610/..., 8×H20, .venv)
# 2026-06-25
set -euo pipefail
...
RUN="mem_space_fifo_b5_chunk512_noleak"
mkdir -p logs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29806 \
  ...
  --use_fifo_memory --fifo_buffer_chunks 5 --fifo_detach \
