#!/usr/bin/env bash
# P8 NULLSINK scale-up arm: chunk_size 512 (compression-ratio ladder rung).
# Mirrors launch_mem_space_p8_nullsink_chunk256_remote196.sh; only chunk_size
# (256 -> 512), run name, master_port differ. Same read-path fix (null/sink slot
# + memory_xattn trainable+saved). Compression-ratio ablation: chunk128 (local) +
# chunk256 (.196) + chunk512 (this) + chunk1024 (diskB-2) form the ladder.
#   - 8 GPUs x bs1 x ga4 = eff batch 32 (matches all sibling arms)
#   - total_steps 5000 (< 1 epoch, no repeats)
# Runs on DISK-B node (28.49.57.76 / 28.59.33.249, share_304376610).
...
RUN="mem_space_perdoc_chunk512_p8_nullsink_diskB"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29790 \
