#!/usr/bin/env bash
# D6 Arm B (2026-06-09): xattn OFF — single-variable ablation isolating the
# dedicated memory cross-attention READ path's contribution.
#
# Identical to launch_mem_space_p8_nullsink_chunk512_diskB.sh (the P11 chunk512
# baseline = D6 Arm A) EXCEPT: drop --use_memory_xattn (and its gate-init flag).
# With xattn OFF the read reverts to the LEGACY KV-prepend path (no dedicated
# own-softmax read, no null/sink — the sink only exists inside the xattn module).
# So Arm B = (xattn OFF). Compare against Arm C (xattn ON, sink OFF) to isolate
# the dedicated-read mechanism, and against Arm A (xattn ON, sink ON) for the
# end-to-end read-path delta.
#
#   - 8 GPUs x bs1 x ga4 = eff batch 32 (matches Arm A / all sibling arms)
#   - total_steps 5000, save_interval 500, eval_interval 0 (inline eval -> NCCL crash)
# Runs on DISK-B node (28.59.33.249 / 28.49.57.76, share_304376610).
set -euo pipefail
...
RUN="d6_xattn_off"
mkdir -p logs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29792 \
