#!/usr/bin/env bash
# EXP-2 (2026-06-11): widen the L3 bottleneck (--l3_n_summary 64 -> 128).
# Identical to launch_mem_space_p11_chunk512_remote196.sh EXCEPT:
#   --l3_n_summary 64 -> 128 (shape change -> from-scratch train, no warm-start)
#   --total_steps 5000 -> 1000  +  --save_interval 500 (new convention)
#   independent RUN / output_dir / log / master_port (29821)
# Runs on diskB node 28.59.33.249 (8x H20). PROJECT_ROOT defaults to diskB root;
# PYBIN defaults to diskB project .venv (torch 2.10+cu128, has transformers).
set -euo pipefail
...
RUN="exp2_l3summary128"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29821 \
  ...
  --dual_gate_tanh_new --use_l3_summary --l3_n_summary 128 --l3_n_layers 2 --l3_n_heads 8 \
