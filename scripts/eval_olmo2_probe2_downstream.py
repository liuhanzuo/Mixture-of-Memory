...
def score_task(model, tok, examples, device, batch_size, add_bos, bos_id,
               pad_id, max_len):
    """Return (n, n_correct_acc, n_correct_accnorm, n_nan, sample_lls).
    ...
    return n, n_correct_acc, n_correct_norm, n_nan, samples, n_trunc
...
def merge(results_dir):
    ...
    _log(f"[merge] {os.path.basename(results_dir.rstrip('/'))}: " +
         " | ".join(f"{t}: acc={v['acc']:.4f} accn={v['acc_norm']:.4f} "
                    f"(n={v['n']},nan={v['n_nan']})" for t, v in tasks.items()))
    return summary
...
def main():
    ...
    if args.prepare_data:
        ...
        _log("[prepare] all datasets cached")
        return
    ...
    for task in tasks:
        ...
        t0 = time.time()
        n, nca, ncn, nnan, samples, ntr = score_task(
        ...
        task_results[task] = {
            ...
            "acc_norm_shard": accn, "seconds": round(dt, 1), "samples": samples,
        }
        _log(f"[shard {args.shard_index}/{args.num_shards}] {task}: n={n} "
             f"acc={acc:.4f} acc_norm={accn:.4f} nan={nnan} trunc={ntr} ({dt:.1f}s)")
