#!/usr/bin/env python3
"""Full fine-tuning with ChunkMemoryBank on Dolmino data.

Supports three modes:
  1. full_finetune + use_memory : Full param update + memory bank (default)
  2. full_finetune + no_memory  : Full param update, no memory (baseline)
  3. LoRA (legacy)              : LoRA only, for reference comparison

Key differences from train_continued_pretrain.py:
- Full fine-tuning: all model parameters trainable, no LoRA/peft wrapper
- use_memory toggle: can disable ChunkMemoryBank for clean baseline comparison
- gradient checkpointing enabled by default for memory efficiency
- Lower default LR (5e-6) for full fine-tuning stability
- KL penalty weight defaults to 0.0 (disabled) for full finetune
...
def main() -> None:
    ...
    if is_main:
        ...
        logger.info("Continued Pretraining: %s / %s", mode_str, mem_str)
        logger.info("=" * 60)
    ...
    if is_main:
        if args.full_finetune:
            logger.info(
                "Building ChunkMemoryModel: FULL FINETUNE, use_memory=%s, grad_ckpt=%s",
                args.use_memory, args.gradient_checkpointing,
            )
        else:
    ...
    if is_main:
        ...
        logger.info(
            "Config: num_slots=%d, top_k=%d, epsilon=%.3f, seq_len=%d, chunks_per_doc=%d",
