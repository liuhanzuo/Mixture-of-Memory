...
MASTER_ADDR="${MASTER_ADDR:-30.203.138.247}"
...
echo "=== Progressive chunk_size chain (RESUME FROM STAGE3) start $(date) ==="

# stage1_c128 + stage2_c256 are already DONE (ckpts present from prior runs).
# Resume the ladder at stage3, warm-starting from the existing stage2 step600 adapter.
STAGE2_CKPT="$OUT_BASE/stage2_c256/mem_space_adapter_step000600.pt"
if [[ ! -f "$STAGE2_CKPT" ]]; then
  echo "[FATAL] stage2 chain ckpt $STAGE2_CKPT missing -> cannot resume at stage3" >&2
  exit 1
fi
echo "Resuming: stage3 warm-start from $STAGE2_CKPT"

# Stage 3: chunk512, warm-start from existing stage2 step600 ckpt.
CKPT3=$(run_stage "stage3_c512" 512 29812 "$STAGE2_CKPT" | tail -1)
# Stage 4: chunk1024, warm-start from stage3.
CKPT4=$(run_stage "stage4_c1024" 1024 29813 "$CKPT3" | tail -1)

echo "=== Progressive chunk_size chain (RESUME FROM STAGE3) DONE $(date) ==="
echo "stage3=$CKPT3"
echo "stage4=$CKPT4"
