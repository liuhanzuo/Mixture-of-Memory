#!/usr/bin/env bash
# Paper B P2.4 post-SFT eval battery for the keep10fresh2 arm on .82 (diskB / zwfy6).
#
# Mirrors scripts/_run_olmo2_p24_eval_sft_keep8_73.sh exactly (same harnesses, same
# tokenisation, same chat_template=False / --add_bos 0 protocol, same 8-GPU
# shard+merge convention), swapping arm/ckpt/output_name.
#
# Pre/post pair (single-variable, SAME node-family + SAME arch = H20 cc9.0 / zwfy6):
#   pre  : outputs/olmo2_probe2_7B_keep10fresh2/step83500.pt
#          -> already scored on THIS disk as 7B_keep10_step83500_v2{,_know}
#             (PPL=12.815922777, core6=0.52999; per-item preds retained).
#             We do NOT re-run pre; we only ASSERT the anchor is complete.
#   post : outputs/olmo2_p24_sft_keep10fresh2/final.pt  (39.01 GB, step=842 keep=10 fresh=2)
#
# ⚠️ CAVEAT (carry into every report): keep10's pre-SFT anchor is step83500 (42% of
# the claimed 200k budget); see status/PAPERB_TABLE4_BUDGET_DEFECT.md. That is FINE
# here because this experiment holds the arm's OWN pre-SFT ckpt fixed and measures
# the SFT delta on it. It is NOT valid for a hypothetical keep10@200k SFT delta.
#
...
# where NAME = "7B_p24_sft_keep10fresh2_final".
#
...
#
# Launch:
#   setsid nohup bash scripts/_run_olmo2_p24_eval_sft_keep10_82.sh \
#     > logs/p24_eval_sft_keep10_82.log 2>&1 &
set -u
...
PRE_NAME="7B_keep10_step83500_v2"
POST_CKPT="outputs/olmo2_p24_sft_keep10fresh2/final.pt"
POST_NAME="7B_p24_sft_keep10fresh2_final"
...
echo "[$(date '+%F %T')] ======== P2.4 keep10fresh2 POST-SFT EVAL BATTERY (.82) START ========"
echo "[$(date '+%F %T')] pre anchor : $PRE_NAME (PPL=12.815923, core6=0.52999)"
echo "[$(date '+%F %T')] post ckpt  : $POST_CKPT"
echo "[$(date '+%F %T')] PREDICTION : dPPL% ~= +13.2% => post-PPL ~= 14.51 (n=3 fit, r=0.998)"
...
echo "[$(date '+%F %T')] ======== P2.4 keep10fresh2 POST-SFT EVAL BATTERY (.82) DONE ========"
