#!/usr/bin/env bash
# 方案B: FIFO hidden-state memory — H2外推 arm, buffer=10 (=5120 tok), 干净版(无泄漏)
...
RUN="mem_space_fifo_b10_chunk512_noleak"
mkdir -p logs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29805 \
  ...
  --use_fifo_memory --fifo_buffer_chunks 10 --fifo_detach \
