#!/usr/bin/env bash
# learnmassbias FINAL ckpt LongEval + RULER W4 eval on .196 (8x H20, shared diskA pool).
# W4 window (mid SWA). Complements B200 W0 + .174 W2 for FINAL-ckpt downstream window curve.
  ...
  while true; do
    ...
    if [ "$kind" = "longeval" ]; then
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_longeval_mem_space.py \
        ...
        --output_name longeval_learnmassFINAL_swa4 --lengths "$L" --num_samples 100 \
        --chunk_size 512 --swa_eval_chunks 4 \
        >>logs/eval_learnmassFINAL_longeval_ruler_swa4_196_gpu${gpu}.log 2>&1
    else
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_ruler_mem_space.py --model_type mem_space \
        --model_path "$MODEL" --checkpoint "$CKPT" --adapter_config "$ACFG" \
        --output_name ruler_learnmassFINAL_swa4 --chunk_size 512 --swa_eval_chunks 4 \
...
wait
echo "LEARNMASS_FINAL_LONGEVAL_RULER_SWA4_196_DONE"
