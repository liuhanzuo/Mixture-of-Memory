#!/usr/bin/env bash
# ROUTE-A selector_temperature sweep — arm3 = selector_temperature=20.
...
# Node: LOCAL 8x H20 (29.162.227.178, diskA share_303098609, .venv/bin/python).
set -euo pipefail
...
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29793 \
  ...
  --selector_temperature 20 --load_balance_weight 0.0 --entropy_aux_weight 0.0 \
