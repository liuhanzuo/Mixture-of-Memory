#!/usr/bin/env bash
# Self-study distillation on PG19 real long books — chunk_size=512, n_ctx=7, num_slots=256.
# N256 variant of launch_distill_pg19_chunk512_nctx7.sh (the 16k-breakthrough recipe).
# Hypothesis: the 32k hard wall is a COMPRESSION-RATIO bottleneck, not a training-length
# one — 32k tokens into 128 slots ~= 250:1. Halving the ratio to 125:1 (N256, top_k
# UNCHANGED at 16) should help attack 32k. N256 is inside the safe slot-capacity band
# (N512/N768 crash; N192/N384 stable + mid-range gains), so 256 is the conservative pick.
#
# Teacher cache is num_slots-INDEPENDENT (frozen backbone logits/hidden), so the existing
# pg19 n_ctx=7 cache distill_cache/pg19_512_nctx7 is REUSED as-is — no rebuild needed.
#
# === USAGE ===================================================================
...
# Run:  setsid bash scripts/launch_distill_pg19_chunk512_N256.sh
# ==============================================================================
set -euo pipefail
...
RUN="distill_pg19_chunk512_N256"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29978 \
  ...
  --chunk_size 512 --batch_size 1 --num_slots 256 --top_k 16 --selector_dim 128 \
