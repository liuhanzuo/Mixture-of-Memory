#!/usr/bin/env bash
# ROUTE-A selector_temperature sweep — arm4 = selector_temperature=80.
# Single-variable vs routeA_arm1_lossfree001 (temp40): ONLY selector_temperature differs.
# All other knobs identical to arm1 (incl. loss_free_update_rate=0.01) for direct compare.
# Base: P11 chunk512 delta-rule + normreadout, 1000-step per-doc fast arm.
# Node: diskB .249 (28.59.33.249, share_304376610, .venv/bin/python). PROJECT_ROOT overridden at invoke.
set -euo pipefail
...
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29794 \
  ...
  --selector_temperature 80 --load_balance_weight 0.0 --entropy_aux_weight 0.0 \
