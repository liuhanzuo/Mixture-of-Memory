#!/usr/bin/env bash
# P8b top_k ablation arm: chunk_size 512 base, single-variable top_k 16 -> 8.
# Copy of launch_mem_space_p8_nullsink_chunk512_diskB.sh; ONLY --top_k (16 -> 8),
# RUN name, and master_port differ. Same read-path fix (null/sink slot +
# memory_xattn trainable+saved, commit c69cd8d). Tests whether sparser routing
# (top_k 8) improves BABILong retrieval vs the chunk512 top_k16 baseline.
#   - 8 GPUs x bs1 x ga4 = eff batch 32 (matches sibling arms)
#   - total_steps 5000 (< 1 epoch, no repeats)
# Runs on DISK-B node (28.49.57.76, share_304376610).
...
RUN="mem_space_p8b_chunk512_topk8_diskB"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29795 \
  ...
  --chunk_size 512 --batch_size 1 --num_slots 128 --top_k 8 --selector_dim 128 \
