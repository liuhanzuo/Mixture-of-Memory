#!/usr/bin/env bash
# T2 chunked associative-recall, chunk_size=512, readout_mass_coef=0.5 (WEAK MASS).
# Fills the LOW end of the mass inverted-U curve (we have coef1/coef2 trained;
# coef4 training on B200; coef0.5 = claimed long-range optimum per readout-attack
# findings but never trained on diskA). Identical to launch_T2_recall_chunk512_MASS.sh
# except RUN name + --readout_mass_coef 0.5. Runs on diskA shared FS (.196 sees data).
set -euo pipefail
...
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29957 \
