#!/bin/bash
# Direction-c MVP: qa5 give-event mixed-context SFT from A-model step2000.
# Goal: fix mem-chain/token-reforward damage to readout (attention anchoring on
# the correct bAbI sf chunk vs PG19 noise). isolation showed mem-chain -33pp.
# Red lines: babilong_mix_fraction 0; all-synthetic (never reads babilong test);
# pg19 background; warm-start from step2000 (NOT a leaked ckpt).
set -euo pipefail
...
echo "[launch] $RUN (qa5 give-event mixed-context SFT, from step2000, lr3e-5, 800steps, mix=0)"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PY -m torch.distributed.run --nproc_per_node=8 --master_port=29813 \
  ...
  --total_steps 800 --lr 3e-5 --warmup_steps 50 \
