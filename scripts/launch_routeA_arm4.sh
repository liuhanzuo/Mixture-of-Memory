#!/usr/bin/env bash
# ROUTE-A balance sweep arm4: identical to launch_mem_space_p11_chunk512_remote196.sh
# EXCEPT adds --use_st_gumbel_topk --st_gumbel_temperature 1.0 (+ total_steps 1000).
# loss_free/entropy/temp all kept at P11 defaults; only the ST-Gumbel top-k flag added.
...
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29813 \
