#!/usr/bin/env bash
# EXP-W1b (2026-06-11): widen write-side fan-out top_k 16 -> 48 on the P11 chunk512
# base. Identical to launch_mem_space_p11_chunk512_remote196.sh EXCEPT:
#   - --top_k 16  ->  --top_k 48
# Goal: stronger version of EXP-W1 (top_k=32) — even more slots written per chunk
# to break the L1 cold-start "rich-get-richer" deadlock. WRITE-SIDE fan-out only;
# selector logic unchanged. Telemetry gate: usage_cov UP AND top1_sim NOT
# collapsed (~0.1-0.2); judge qa1/qa5 8k-32k vs P11 base.
# Runs on diskB node 28.59.33.249 (8x H20, share_304376610 FS, project .venv).
set -euo pipefail
...
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29827 \
  ...
  --chunk_size 512 --batch_size 4 --num_slots 128 --top_k 48 --selector_dim 128 \
