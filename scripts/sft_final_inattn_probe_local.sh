...
CKPT="outputs/sft_unfreeze_inattn_full/full_model.pt"
...
for s in 0 1 2 3; do
  CUDA_VISIBLE_DEVICES=$s nohup $PYTHON_BIN -m scripts.eval_ruler_mem_space \
    ...
    --results_folder babilong_results --output_name sft_final_probe_off \
    > logs/sft_final_probe_off_shard${s}.log 2>&1 &
done
...
for s in 0 1 2 3; do
  ...
  CUDA_VISIBLE_DEVICES=$gpu nohup $PYTHON_BIN -m scripts.eval_ruler_mem_space \
    ...
    --results_folder babilong_results --output_name sft_final_probe_oracle \
    > logs/sft_final_probe_oracle_shard${s}.log 2>&1 &
done
