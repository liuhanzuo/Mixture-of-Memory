#!/usr/bin/env bash
# T2 chunked associative-recall (readout-pivot, 2026-06-14) — chunk_size=1024.
...
# status/STAGING_REPORT_readout_pivot.md). chunk_size=1024 -> n_ctx=7 (gap 7168).
# NOTE: eval must use the SAME --chunk_size 1024 to match memory write granularity.
set -euo pipefail
...
RUN="T2_recall_chunk1024_seq16k_B200bs8s125_seed1234_N128"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29954 \
  ...
  --t2_recall_mix_fraction 0.5 --t2_num_keys 1 --t2_gap_tokens 15360 \
