#!/usr/bin/env bash
# nctx63 PG19 distillation + agreement-gated student-confidence weighting.
# Only teacher/student argmax-agreeing tokens receive student-confidence reweighting.
set -euo pipefail
...
RUN="distill_pg19_chunk512_nctx63_studentconf"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29992 \
  ...
  --distill_conf_weighting student \
