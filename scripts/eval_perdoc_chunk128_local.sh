#!/usr/bin/env bash
# Offline BABILong eval for per-doc chunk128 arm (mem_space_perdoc_chunk128, route_aux=0).
# Round3 chunk_size ablation: chunk128 (local) vs chunk256 (remote 28.59.80.196).
# Gate: >=4k accuracy vs the ~1-2% noise floor P1/P2 arms hit. One length per GPU across 8 H20.
set -uo pipefail
...
CKPT_DIR=outputs/mem_space_perdoc_chunk128
ADAPTER_CONFIG=${CKPT_DIR}/adapter_config.json
CKPT=${CKPT_DIR}/mem_space_adapter.pt
RESULTS=babilong_results/perdoc_chunk128_local
TASKS="qa1 qa2 qa5"
LOGDIR=logs/eval_perdoc_chunk128_local
...
for L in "${LENGTHS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=$GPU setsid bash -c "$PYBIN scripts/run_babilong_mem_space.py \
    ...
    --results_folder $RESULTS --output_name perdoc_chunk128_${L} \
