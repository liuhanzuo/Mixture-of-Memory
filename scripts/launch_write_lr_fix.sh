#!/bin/bash
cd /apdcephfs_wzc1/share_303098609/pighzliu_code/Mixture-of-Memory

# write_lr fix: gradient-preserving delta-rule write
# Root cause: write_lr=1.0 → ∂updated/∂slots=0 → gradient chain broken across chunks
# Fix: write_lr=0.1 → preserves ~3.4% gradient over 32 chunks
# This should enable NIAH training signal to propagate back to early chunks

torchrun --nproc_per_node=8 scripts/train_cross_attn_memory.py \
    --model /apdcephfs_wzc1/share_303098609/pighzliu_code/models/Llama--Llama3-8b \
    --shard_dir /apdcephfs_wzc1/share_303098609/pighzliu_code/data/dolmino-mix-1124-llama3 \
    --output_dir outputs/write_lr_0.1 \
    --wikitext_path data/wikitext_chunks_llama3_4096.npy \
    --niah_data data/pg19_chunks_llama3.npy \
    --niah_mix_fraction 0.5 \
    --niah_max_N 2 \
    --niah_warmup_steps 100 \
    --lambda_retrieve 1.0 \
    --num_shards 25 \
    --seq_len 4096 \
    --chunks_per_doc 32 \
    --num_slots 64 \
    --top_k 8 \
    --use_cross_attn_memory \
    --residual_scale 1.0 \
    --write_lr 0.1 \
    --cross_attn_lr_factor 10 \
    --full_finetune \
    --use_memory \
    --gradient_checkpointing \
    --gradient_accumulation_steps 4 \
    --lr 5e-6 \
    --min_lr 1e-6 \
    --warmup_steps 200 \
    --max_steps 2000 \
    --eval_interval 100 \
    --save_interval 200 \
    --weight_decay 0.01 \
    --max_grad_norm 1.0
