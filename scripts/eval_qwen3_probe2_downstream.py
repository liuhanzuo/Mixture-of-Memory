#!/usr/bin/env python3
"""Likelihood-based multiple-choice DOWNSTREAM eval for the Qwen3 prune-then-heal
probe (Paper B P2.3, cross-family control for the OLMo-2 prune-then-heal
dissociation). Companion to scripts/eval_qwen3_probe2_ppl.py.

Ported VERBATIM from scripts/eval_olmo2_probe2_downstream.py (2026-08-02). The
ONLY change is the import source of the model-construction helpers: this script
imports load_base_model / load_pruned_model / _log from eval_qwen3_probe2_ppl
(Qwen3 family) instead of eval_olmo2_probe2_ppl (OLMo-2 family). All dataset
loading, LL-based MC scoring, sharding and merge logic is family-agnostic and
UNCHANGED, preserving the Paper B base-protocol (chat_template=False, no BOS by
default, LL-based MC argmax).

Question this answers
---------------------
The Qwen3 held-out PPL ledger shows the pruned+healed Qwen3-8B recovers
language-modelling. But PPL is not capability. Here we measure zero-shot
**downstream MC accuracy** (NO generation -- for each candidate answer we score
the teacher-forced sum log-prob of the continuation tokens and take argmax) to
test whether the dropped top layers were redundant (capability recovers with PPL)
or carried knowledge/reasoning that lags PPL -- the same PPL/MMLU dissociation
test we ran on OLMo-2, now on Qwen3 for a cross-family comparison.

Arch construction: NO drift. We import load_pruned_model / load_base_model from
eval_qwen3_probe2_ppl.py (which aligns with the trainer's build), so the shell is
rebuilt + strict-loaded identically to the PPL eval.
...
* add_special_tokens=False (no auto-BOS), matching the Paper B base-protocol and
  how published lm-eval numbers are made.
...
def main():
    ...
    p.add_argument("--add_bos", type=int, default=0,
                   help="0 (default, Paper B base-protocol / matches lm-eval) or 1 to prepend BOS")
    p.add_argument("--limit", type=int, default=0,
                   help=">0 caps examples per task (post-strided); sanity only")
    p.add_argument("--output_name", type=str, required=False)
    p.add_argument("--results_root", type=str, default="qwen3_probe2_downstream_results")
