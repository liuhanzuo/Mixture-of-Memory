#!/bin/bash
# Beacon Pyramid (idea #3) REAL TRAIN — multi-scale near-fine/far-coarse.
# Reader (unfrozen {16,28-31}) + beacon pool trained JOINTLY to consume the
# compressed multi-scale FIFO prefix (Activation-Beacon-style). 400 steps.
# ARG1=nproc ARG2=master_port ARG3=CUDA_VISIBLE_DEVICES ARG4=run_suffix
# Red lines: mix=0, clean A-model step2000 warm-start, buffer25/chunk512.
set -euo pipefail
...
NPROC="${1:-4}"; MPORT="${2:-29851}"; CVD="${3:-0,1,2,3}"; SUF="${4:-}"
RUN="mem_space_beacon_pyramid${SUF}"
mkdir -p logs
...
echo "[launch] $RUN (pyramid fine2/mid6/branch4, nproc=$NPROC dev=$CVD, 400 steps, mix=0)"
