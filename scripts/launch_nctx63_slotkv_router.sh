#!/usr/bin/env bash
# Per-slot raw-KV cache n_ctx=63 arm: train with router (learned selector) slot-KV selection.
# Completes the train-time selector trichotomy: recency (healthy) / all (NaN-crashed) / router (this).
set -euo pipefail
...
RUN="slot_kv_cache_pg19_chunk512_nctx63_router"
mkdir -p logs outputs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29990 \
  ...
  --use_slot_kv_cache --slot_kv_cache_layer 16 --slot_kv_select_mode router \
