#!/bin/bash
# Approach B: Reconstruction Loss (--memory_init=learnable --recon_loss_weight=1.0)
# Learnable embeddings + auxiliary loss forcing slots to reconstruct hidden states
# Creates explicit pressure for slots to carry information
...
LOGDIR=logs
mkdir -p $LOGDIR
LOGFILE=$LOGDIR/approach_b_recon_$(date +%Y%m%d_%H%M).log
...
torchrun \
    ...
    --output_dir /apdcephfs_wzc1/share_303098609/pighzliu_code/Mixture-of-Memory/outputs/approach_b_recon \
