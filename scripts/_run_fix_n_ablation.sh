#!/bin/bash
# fix_n_ablation: Fix routing collapse caused by SKRL disabled + strong load balance weight.
#
# Root cause of fix_j_l_m_ablation routing collapse:
#   top1_sim = 1/N = 0.00195 for all 1000+ steps — routing never differentiates.
#   Two culprits:
#     (1) --skrl_weight 0.0: SKRL disabled → no symmetry-breaking gradient on slot_keys
#     (2) --load_balance_weight 0.01: actively enforces uniform routing
#
# Fix N changes vs fix_j_l_m_ablation:
#   - Re-enable SKRL with varied weights across nodes (symmetry-breaking gradient)
#   - Reduce load_balance_weight by 10× (0.01 → 0.001)
...
#   Fix N:   SKRL re-enabled (variable weight), load_balance 10× lower  ← NEW
#
# Ablation sweep: varied SKRL weights, fixed sigma=0.01
#   NODE_IDX 0 → b200-2: skrl_weight=0.05
#   NODE_IDX 1 → b200-3: skrl_weight=0.01
#   NODE_IDX 2 → b200-4: skrl_weight=0.10
#
# Success criterion:
...
#
# Usage:
#     NODE_IDX=0 bash scripts/_run_fix_n_ablation.sh   # b200-2
#     NODE_IDX=1 bash scripts/_run_fix_n_ablation.sh   # b200-3
#     NODE_IDX=2 bash scripts/_run_fix_n_ablation.sh   # b200-4
...
TAG="fix_n_ablation_node${NODE_IDX}"
OUTPUT_DIR="$PROJECT_DIR/outputs/fix_n_ablation_node${NODE_IDX}"
mkdir -p "$OUTPUT_DIR"
...
# Fixed sigma for all nodes (Fix N: sigma no longer ablated)
SIGMA=0.01

# Select SKRL weight by node (Fix N: re-enable SKRL with varied weights)
if   [ "$NODE_IDX" = "0" ]; then SKRL_W=0.05
elif [ "$NODE_IDX" = "1" ]; then SKRL_W=0.01
else                              SKRL_W=0.10
fi
...
echo "=== fix_n_ablation  NODE_IDX=${NODE_IDX}  sigma=${SIGMA}  skrl_weight=${SKRL_W}  port=${PORT} ==="
echo "Log: $LOG_FILE"
...
echo "=== fix_n_ablation NODE_IDX=${NODE_IDX} DONE ==="
