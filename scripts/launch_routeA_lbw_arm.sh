#!/usr/bin/env bash
# ROUTE-A load_balance_weight sweep — closes the last unswept routing knob.
# Single-variable vs routeA_arm1 (loss_free, temp40, lbw=0.0): ONLY load_balance_weight differs.
# All other knobs identical to arm1 for direct compare.
# Base: P11 chunk512 delta-rule + normreadout, 1000-step per-doc fast arm.
# Multi-seed: launch with SEED=42 (diskA .196) and SEED=1234 (diskB .76) per multi-seed iron rule.
# Env knobs: PROJECT_ROOT, PYTHON_BIN, RUN, SEED, LBW, MASTER_PORT.
set -euo pipefail
...
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=$MASTER_PORT \
  ...
  --selector_temperature 40 --load_balance_weight $LBW --entropy_aux_weight 0.0 \
  --use_loss_free_balance --loss_free_update_rate 0.01 --num_global_slots 4 \
  ...
  --wandb_run_name $RUN --dtype bfloat16 --attn_impl sdpa --seed $SEED" \
