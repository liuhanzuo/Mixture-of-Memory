#!/usr/bin/env bash
# P11 ablation on adopted P11 base: chunk_size 512 -> 256 (single variable).
# Identical to launch_mem_space_p11_chunk512_remote196.sh EXCEPT --chunk_size 256, RUN, master_port.
# Completes the P11 chunk ladder: 256 (this) / 512 (adopted baseline, done) / 1024 (running local).
...
RUN="mem_space_p11_chunk256_deltarule_normreadout"
# H20 batch tuning (2026-06-07): physical bs4 x grad_accum1 x 8gpu = eff_batch 32, held
# constant vs prior bs1 x grad_accum4. chunk256 < chunk512, so bs4 fits with ample margin
# (chunk512 bs4=71GiB; chunk256 lower). Identical optimization dynamics (bs2==2xbs1 verified rel<1e-4).
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29793 \
  ...
  --chunk_size 256 --batch_size 4 --num_slots 128 --top_k 16 --selector_dim 128 \
