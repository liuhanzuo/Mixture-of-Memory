#!/bin/bash
# Experiment D: Cross-Chunk Slot Propagation
# Based on Approach C (strided), with --cross_chunk_propagation to carry slots
# across chunks within a document instead of re-initializing every chunk.
set -e
...
LOGDIR=logs
mkdir -p $LOGDIR
LOGFILE=$LOGDIR/experiment_d_cross_chunk_$(date +%Y%m%d_%H%M).log
...
torchrun \
    ...
    --output_dir /apdcephfs_wzc1/share_303098609/pighzliu_code/Mixture-of-Memory/outputs/experiment_d_cross_chunk \
