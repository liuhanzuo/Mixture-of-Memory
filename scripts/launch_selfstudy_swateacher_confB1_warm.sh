...
RUN="${RUN:-mem_space_selfstudy_swateacher_confB1_warm_chunk512}"
NPROC="${NPROC:-8}"
...
setsid nohup bash -c "CUDA_VISIBLE_DEVICES=$GPUS $PYBIN -m torch.distributed.run --nproc_per_node=$NPROC --master_port=$MASTER_PORT \
  ...
  --init_checkpoint outputs/mem_space_selfstudy_rawkv_chunk512/full_model_step000500.pt \
  --init_adapter_config outputs/mem_space_selfstudy_rawkv_chunk512/adapter_config.json \
  ...
  --distill_conf_weighting teacher --distill_conf_metric margin --distill_conf_temp 0.1 \
  --distill_conf_min 0.25 --distill_conf_max 2.0 \
