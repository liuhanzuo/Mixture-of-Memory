#!/bin/bash
# UNFREEZE-SWEEP arm2 on REMOTE diskB (.53): unfreeze contiguous TOP-16 layers (16..31).
# GPUs 0-3, DDP + ZeRO-1 (shards the ~32GB AdamW state across ranks; params/grads
# stay replicated so the t2_select salience helper works — FSDP resharded them
# to 0-size and crashed). Only variable vs arm1 = unfreeze set.
set -euo pipefail
...
RUN="mem_space_unfreeze_arm2_top16"
mkdir -p logs
...
echo "[launch] $RUN (unfreeze TOP-16 layers 16..31, DDP+ZeRO-1 GPUs 0-3, from step2000, 400steps, mix=0)"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3 $PY -m torch.distributed.run --nproc_per_node=4 --master_port=29814 \
