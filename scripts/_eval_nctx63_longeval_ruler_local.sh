  ...
  while true; do
    ...
    if [ "$kind" = "longeval" ]; then
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_longeval_mem_space.py \
        ...
        --output_name longeval_nctx63_s250 --lengths "$L" --num_samples 100 \
        --chunk_size 512 --swa_eval_chunks 0 \
        >>logs/eval_nctx63_longeval_ruler_gpu${gpu}.log 2>&1
    else
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_ruler_mem_space.py --model_type mem_space \
        --model_path "$MODEL" --checkpoint "$CKPT" --adapter_config "$ACFG" \
        --output_name ruler_nctx63_s250 --chunk_size 512 --swa_eval_chunks 0 \
...
for g in 4 5 6 7; do worker $g & done
wait
echo "NCTX63_LONGEVAL_RULER_DONE"
