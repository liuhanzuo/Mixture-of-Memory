#!/usr/bin/env bash
# P11 chunk512 + token-mass readout bias ablation (2026-06-18).
# Identical to launch_mem_space_p11_chunk512_remote196.sh EXCEPT:
#   + --use_readout_mass_bias --readout_mass_coef 1.0
#   + total_steps 1000 (heartbeat default), save_interval 500
# Mechanism-side readout knob: adds readout_mass_coef*log1p(slot_token_mass) to
# the read softmax so slots that absorbed more raw tokens get read more often.
# Distinct from the previously-falsified training-side distillation mass.
# Runs on diskA (local 8x H20). torch-base env has transformers.
set -euo pipefail
...
RUN="${RUN:-mem_space_p11_chunk512_massbias_coef1}"
MASS_COEF="${MASS_COEF:-1.0}"
MASTER_PORT="${MASTER_PORT:-29951}"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=$MASTER_PORT \
  ...
  --use_readout_mass_bias --readout_mass_coef $MASS_COEF \
