#!/usr/bin/env bash
# P2 decoupled-read arm (local H20-1): IDENTICAL to dolmino_norecon_local_v2 control
# but with --use_decoupled_read (standalone cross-attn read, bypasses prepend-softmax
# dilution). Single-variable comparison vs P1 norecon baseline.
# Gate: offline BABILong >=4k must cross the ~1-2% noise floor that BOTH P1 arms hit.
...
RUN="dolmino_p2_decoupled_local"
setsid bash -c "$PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29771 \
