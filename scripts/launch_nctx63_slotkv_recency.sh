#!/usr/bin/env bash
# Per-slot raw-KV cache n_ctx=63 arm: train with recency slot-KV selection.
# Based on launch_slot_kv_cache_test.sh. Do not run automatically.
set -euo pipefail
...
RUN="slot_kv_cache_pg19_chunk512_nctx63_recency"
mkdir -p logs outputs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29988 \
  ...
  --use_slot_kv_cache --slot_kv_cache_layer 16 --slot_kv_select_mode recency \
