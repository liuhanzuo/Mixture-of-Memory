#!/usr/bin/env bash
# nctx63 PG19 distillation + teacher-confidence weighting (offline full-context teacher cache).
# Based on launch_distill_pg19_chunk512_nctx63.sh; default behavior remains off elsewhere.
set -euo pipefail
...
RUN="distill_pg19_chunk512_nctx63_teacherconf"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29991 \
  ...
  --distill_conf_weighting teacher \
