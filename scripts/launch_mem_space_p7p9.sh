#!/usr/bin/env bash
# P7 (loss-free balancing) + P9 (always-on register slots) Dolmino CPT arm.
# Built from the routeaux arm (launch_mem_space_perdoc_chunk128_routeaux_remote.sh)
# which gave the best BABILong so far. Differences vs that arm:
#   P7: --use_loss_free_balance + --loss_free_update_rate 0.001, and
#       --load_balance_weight 0.0 (loss-free bias REPLACES the uniform-pushing aux;
#       the two must not both be nonzero per config.py docstring). entropy_aux also 0.
#   P9: --num_global_slots 4  (always-on register slots absorb attention-sink so the
#       128 addressable slots do real routing). No new code (config.py:76 existing arg).
#   route_aux kept ON (1.0) since R3-2 showed it lifts memory-task accuracy.
# eval_interval=0 (inline BABILong eval -> NCCL SIGABRT, CODEBUDDY.md).
...
RUN="mem_space_perdoc_chunk128_p7p9"
