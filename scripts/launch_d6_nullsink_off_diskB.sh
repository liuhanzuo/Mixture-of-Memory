#!/usr/bin/env bash
# D6 Arm C (2026-06-09): null-sink OFF — single-variable ablation isolating the
# learnable null/sink slot's contribution.
#
# Identical to launch_mem_space_p8_nullsink_chunk512_diskB.sh (the P11 chunk512
# baseline = D6 Arm A) EXCEPT: add --memory_xattn_disable_null_sink. xattn stays
# ON (dedicated own-softmax read), but the read softmax has NO "attend to
# nothing" escape column. Compare against Arm A (xattn ON, sink ON) to isolate
# the null-sink, holding the dedicated-read mechanism fixed.
#
...
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29793 \
  ...
  --use_memory_xattn --memory_xattn_gate_init 0.4 --memory_xattn_disable_null_sink \
