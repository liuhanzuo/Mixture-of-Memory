#!/usr/bin/env bash
# nctx63 SOTA LongEval + RULER W0 eval on local GPU 4-7 (idle group).
...
CKPT="$ROOT/outputs/distill_pg19_nctx63_lr5e5_s1234/mem_space_adapter_step000250.pt"
ACFG="$ROOT/outputs/distill_pg19_nctx63_lr5e5_s1234/adapter_config.json"
MODEL="$ROOT/models/Meta-Llama-3-8B"
mkdir -p logs

# Build a shared pool of (kind task length gpu-agnostic) cells; 4 GPUs (4,5,6,7) pop atomically.
POOL=$(mktemp); LOCK=$(mktemp)
# LongEval lengths
for L in 4k 8k 16k 32k; do echo "longeval - $L" >> "$POOL"; done
# RULER tasks x lengths
  ...
  while true; do
    ...
    if [ "$kind" = "longeval" ]; then
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_longeval_mem_space.py \
        ...
        --output_name longeval_lr5e5_s1234_s250 --lengths "$L" --num_samples 100 \
        --chunk_size 512 --swa_eval_chunks 0 \
        >>logs/eval_lr5e5_longeval_ruler_gpu${gpu}.log 2>&1
    else
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_ruler_mem_space.py --model_type mem_space \
        --model_path "$MODEL" --checkpoint "$CKPT" --adapter_config "$ACFG" \
        --output_name ruler_lr5e5_s1234_s250 --chunk_size 512 --swa_eval_chunks 0 \
...
wait
echo "LR5E5_LONGEVAL_RULER_DONE"
