#!/usr/bin/env bash
# L3-isolation ablation, NO-L3 arm (local H20-1): byte-identical to
# launch_dolmino_p2_decoupled_local.sh EXCEPT --use_l3_summary -> --no_l3_summary.
# Pairs with the WITH-L3 arm (dolmino_p2_decoupled_local / remote H20-2) so the
# ONLY variable is the L3 summary pool. L2 is off in both (config default use_l2=False).
# Same seed=42, same data, same hyperparams. See versions/v15_decoupled_read.md.
set -euo pipefail
...
RUN="l3iso_noL3_local"
setsid bash -c "$PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29781 \
  ...
  --dual_gate_tanh_new --no_l3_summary --l3_n_summary 64 --l3_n_layers 2 --l3_n_heads 8 \
