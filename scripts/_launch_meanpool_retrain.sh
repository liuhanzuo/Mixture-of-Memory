#!/usr/bin/env bash
# mean-pool salience 重训(2026-06-29): 修q_last架构缺陷。MEM_SALIENCE_QPOOL=mean(训练推理一致)
# mean-pool纯推理就把reader-attn recall@4从0.17→0.40(追近bm25)。重训应更高。
# 从step2000续训, mix=0, format hard-neg信号(L3, selector有梯度)。@.196 diskA(代码直接可见)
set -euo pipefail
...
export MEM_SALIENCE_QPOOL=mean   # ★关键: mean-pool探针(训练+后续eval都要设此env)
PY="$R/.venv/bin/python"
RUN="mem_space_select_meanpool_fmt_g8k"
...
echo "[launch] $RUN — MEM_SALIENCE_QPOOL=mean + format hard-neg + gap8192 curriculum, step2000续训, mix=0"
setsid bash -c "MEM_SALIENCE_QPOOL=mean CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PY -m torch.distributed.run --nproc_per_node=8 --master_port=29821 \
  ...
  --t2_hard_distractors 3 --t2_hard_distractor_mode format \
  ...
  --grad_clip 1.0 --proj_grad_clip 0.1 --wandb_project mixture-of-memory --wandb_run_name $RUN --dtype bfloat16 --attn_impl sdpa --seed 46" \
