#!/bin/bash
# Launch MemoryLLM-8B-chat BABILong evaluation on an H20 node.
# Usage:
#   bash scripts/launch_h20_memoryllm.sh                 # default 6 length bins on GPUs 0-5
#   bash scripts/launch_h20_memoryllm.sh 1k:6 2k:7 ...  # custom length:gpu mapping
...
MODEL_PATH="${MODEL_PATH:-${PROJECT_ROOT}/baselines/memoryllm-8b-chat-hf}"
RESULTS="${RESULTS:-${WORKDIR}/babilong_results}"
OUTPUT_NAME="${OUTPUT_NAME:-MemoryLLM-8B-chat}"
...
echo "Launched ${#LENGTHS_GPU[@]} MemoryLLM jobs"
