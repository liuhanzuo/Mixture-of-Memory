#!/usr/bin/env bash
# n_ctx=63 PG19 distillation arm with only 16 memory slots and top_k=16.
...
RUN="distill_pg19_nctx63_N16_top16"
...
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29992 \
  ...
  --chunk_size 512 --batch_size 1 --num_slots 16 --top_k 16 --selector_dim 128 \
