#!/usr/bin/env bash
# P8 NULLSINK scale-up arm: chunk_size 1024 (top rung of compression-ratio ladder).
# Mirrors launch_mem_space_p8_nullsink_chunk256_remote196.sh; only chunk_size
# (256 -> 1024), run name, master_port differ. Same read-path fix (null/sink slot
# + memory_xattn trainable+saved). Plan note: chunk1024(slot4096) >> chunk128
# (qa1_0k 89 vs 55) in an earlier run -> this fills the P8-nullsink ladder at 1024.
#   - 8 GPUs x bs1 x ga4 = eff batch 32 (matches all sibling arms)
#   - total_steps 5000 (< 1 epoch, no repeats)
# Runs on DISK-B node (28.59.33.249, share_304376610). chunk1024 is the longest
# span -> watch activation memory; gradient_checkpointing on, bs1.
# eval_interval=0 (inline BABILong -> NCCL SIGABRT). Offline eval is the judge.
set -euo pipefail
...
RUN="mem_space_perdoc_chunk1024_p8_nullsink_diskB"
