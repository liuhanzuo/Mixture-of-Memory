#!/usr/bin/env bash
# 8-GPU sharded likelihood-MC DOWNSTREAM eval for the OLMo-2 prune-then-heal probe
# (Paper B, direction #4). EVAL-ONLY (forward-only) on .73. Companion to
# scripts/_run_olmo2_probe2_ppl_8gpu.sh.
#
...
BS=16
DONE=logs/olmo2_downstream_DONE
rm -f "$DONE"
...
CONFIGS=(
  ...
  "1B_keep7_step${LATEST_1B_STEP}|../models/OLMo-2-0425-1B|${LATEST_1B}"
...
# prepare datasets once (single process, populates HF cache)
echo "[$(date '+%F %T')] prepare_data (cache all tasks)"
$PY scripts/eval_olmo2_probe2_downstream.py --prepare_data --tasks "$TASKS" \
  > logs/olmo2_downstream_prepare.log 2>&1
tail -8 logs/olmo2_downstream_prepare.log
