#!/usr/bin/env bash
# combined_s250 LongEval + RULER at W4-SWA on .196 (diskA, shares FS with local).
# BABILong: combined arm W4 COLLAPSED (qa1>=1k all 0, comma-spam). This eval tests
# whether that collapse window also fails downstream LongEval/RULER, completing the
# combined-arm window-transfer matrix: W0(done) / W2(local) / W4(this) / W6(B200).
set -uo pipefail
  ...
  while true; do
    ...
    if [ "$kind" = "longeval" ]; then
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_longeval_mem_space.py \
        ...
        --output_name longeval_combined_s250_swa4 --lengths "$L" --num_samples 100 \
        --chunk_size 512 --swa_eval_chunks 4 \
        >>logs/eval_combined_longeval_ruler_swa4_gpu${gpu}.log 2>&1
    else
      CUDA_VISIBLE_DEVICES=$gpu $PY scripts/eval_ruler_mem_space.py --model_type mem_space \
        --model_path "$MODEL" --checkpoint "$CKPT" --adapter_config "$ACFG" \
        --output_name ruler_combined_s250_swa4 --chunk_size 512 --swa_eval_chunks 4 \
...
wait
echo "COMBINED_LONGEVAL_RULER_SWA4_DONE"
