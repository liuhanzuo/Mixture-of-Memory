#!/usr/bin/env bash
# P10 ablation on P8 chunk512 base: key_repulsion 1.0 -> 0.05 + ST-Gumbel top-k.
# Identical to launch_mem_space_p8_nullsink_chunk512_diskB.sh EXCEPT:
#   --key_repulsion_weight 1.0 -> 0.05  and  + --use_st_gumbel_topk
# Runs on diskA LOCAL node (8x H20, share_303098609). Offline eval is the judge.
set -euo pipefail
...
RUN="mem_space_p10_chunk512_keyrep005_stgumbel"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29791 \
  ...
  --key_repulsion_weight 0.05 --key_repulsion_threshold 0.3 --slot_value_norm_cap 5.0 \
  --use_st_gumbel_topk \
