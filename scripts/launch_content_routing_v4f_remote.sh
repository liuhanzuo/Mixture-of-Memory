#!/bin/bash
# v4f remote: content-based routing + TBPTT + key_repulsion_weight=0.05
# Key change vs v4e: key_repulsion_weight=0.05 to stabilize key diversity
# v4e showed bimodal oscillation (key_max_cos 0.35↔0.92) without repulsion
set -e
...
OUTPUT_DIR=outputs/content_routing_v4f_remote
LOGFILE=logs/content_routing_v4f_remote.log
...
$PYTHON -m torch.distributed.run --nproc_per_node=8 \
    ...
    --wandb_run_name content_routing_v4f_remote \
    --dtype bfloat16 \
    --attn_impl sdpa \
    --seed 44 \
    2>&1 | tee $LOGFILE
