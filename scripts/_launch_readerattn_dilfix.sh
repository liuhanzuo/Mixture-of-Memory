#!/usr/bin/env bash
# reader-attn dilution修复重训 (方案2, 2026-06-29)
# 攻主因: 训练n_ctx=7 vs eval buffer=25不匹配。改t2_gap8192(n_ctx16)+curriculum渐增到24(接近eval25)。
# 从step2000续训(warm-start), mix=0, 新RUN名(不覆盖step2000)。
set -euo pipefail
...
echo "[launch] $RUN — dilution修复: t2_gap8192(n_ctx16)+curriculum0:4,300:8,600:16,1000:24, 从step2000续训, mix=0"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PY -m torch.distributed.run --nproc_per_node=8 --master_port=29811 \
