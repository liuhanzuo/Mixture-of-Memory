#!/usr/bin/env bash
# L2 (continuous token-compressed KV pool readout) + pg19 long-document data.
# Based on launch_expL2ON_N128.sh (L2 config: --use_l2 + slot128/topk16 + L3 +
# use_memory_xattn + use_delta_rule_writeback --normalize_readout), but swaps the
# SHORT dolmino per-doc data for pg19 per-book (min 8k tokens, 78% >=32k) so L2
# learns continuous compression on TRUE long sequences. curriculum 0:7 ->
# window (7+1)*512=4096 (pg19 books are >=8k so no per-doc starvation).
# Runs on diskB node 28.59.33.249 (8x H20). Uses project .venv (torch2.10+cu128).
set -euo pipefail
...
RUN="expL2ON_pg19_N128"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29904 \
  ...
  --use_delta_rule_writeback --normalize_readout --use_l2 \
  ...
  --curriculum 0:7 --bptt_window 2 --no_detach_slots_in_selector \
