#!/bin/bash
# UNFREEZE-SWEEP arm3: FULL unfreeze (all 32 decoder layers + norm + lm_head).
# Only variable vs arm1/arm2 is the unfreeze set (=everything). FSDP shards the
# ~7B optimizer state across 8 ranks (DDP would OOM). Runs on REMOTE diskB node.
# Red lines: babilong_mix_fraction 0 (all-synthetic); pg19 background;
# warm-start from A-model step2000 (NOT a leaked ckpt).
set -euo pipefail
...
echo "[launch] $RUN (FULL unfreeze all 32 layers, FSDP, from step2000, lr3e-5, 400steps, mix=0)"
