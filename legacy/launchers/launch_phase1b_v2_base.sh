#!/usr/bin/env bash
# Phase-1B v2-style run on raw Llama-3.2-1B base.
#
# Purpose:
#   Keep the phase1b v2 L1+L3 training recipe, but swap the backbone from
#   Llama-3.2-1B-Instruct to raw Llama-3.2-1B. This is the direct training-side
#   comparison the user asked for against LM2-matched base-model experiments.
#
# Recipe:
#   * phase1b v2-style L1+L3 + dual gate + shared bank
#   * tasks=qa1,qa2,qa5; lengths=1k,2k,4k; pg19_mix=0.20
#   * 5000 total steps; save every 500 steps
#   * NO chat template (training script explicitly says that is for Instruct backbones)
#
# Launch:
#   bash scripts/launch_phase1b_v2_base.sh
#
# Outputs:
#   outputs/babilong_sft_phase1b_v2_llama32_1b_base_5k/
#
# Log:
#   logs/phase1b_v2_llama32_1b_base_5k_<timestamp>.log
...
BACKBONE_PATH="${BACKBONE_PATH:-/apdcephfs_wzc1/share_303098609/pighzliu_code/models/Llama-3.2-1B}"
