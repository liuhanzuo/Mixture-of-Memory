#!/usr/bin/env bash
# Writeback-mode arm A (lowrank_gate) on remote H20-2 (28.59.80.196):
# slot_dim=16384, 128 slots, low-rank gate (r=256) replacing the LM2 dual_gate
# (which would OOM at slot_dim=16384). Remote env: torch-base python, WANDB offline.
set -euo pipefail
...
setsid nohup bash -c "$PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29781 \
...
echo "launched $RUN pid=$! on H20-2"
