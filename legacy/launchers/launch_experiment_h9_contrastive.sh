#!/bin/bash
# Experiment H9: H7 + Contrastive Retrieval Supervision
#
# Hypothesis: NIAH accuracy is 0% because the LM loss alone is too weak to
# teach the model to READ from specific memory slots. Adding InfoNCE contrastive
# loss directly supervises the query->slot attention to focus on the needle slot.
#
# Changes vs H7:
#   - contrastive_weight: 0.0 → 1.0 (InfoNCE retrieval supervision)
...
LOGDIR=logs
mkdir -p $LOGDIR
LOGFILE=$LOGDIR/experiment_h9_contrastive_$(date +%Y%m%d_%H%M).log

echo "============================================"
echo "Experiment H9: H7 + Contrastive Retrieval"
echo "  Write layer: 16, Read layers: 18,22,26,30"
echo "  Slots: 64, Dual-gate: ON"
echo "  NIAH: lambda=0.1, mix=0.10, warmup=500"
echo "  CONTRASTIVE WEIGHT: 1.0 (NEW!)"
echo "Output: outputs/experiment_h9_contrastive"
echo "Log:    $LOGFILE"
echo "============================================"
...
torchrun \
    ...
    --output_dir /apdcephfs_wzc1/share_303098609/pighzliu_code/Mixture-of-Memory/outputs/experiment_h9_contrastive \
