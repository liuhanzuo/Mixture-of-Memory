#!/usr/bin/env bash
# EXP-1 (2026-06-11): turn ON L3 summary-token diversity regularization.
# Identical to launch_mem_space_p11_chunk512_remote196.sh EXCEPT:
#   --l3_diversity_weight 0.0 -> 0.1   (open the dormant v9/v10 L3 diversity loss)
#   --total_steps 5000 -> 1000  +  --save_interval 500 (1k>500 才续训, new convention)
#   independent RUN / output_dir / log / master_port (29820)
...
RUN="exp1_l3div01"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29820 \
  ...
  --multi_query_tau 1.0 --l3_diversity_weight 0.1 --l3_diversity_threshold 0.5 \
