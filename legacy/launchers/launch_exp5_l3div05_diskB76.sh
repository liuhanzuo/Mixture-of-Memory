#!/usr/bin/env bash
# EXP-5 (2026-06-11): L3 summary-token diversity regularization, STRONGEST weight arm.
# Companion arm to EXP-1 (l3_diversity_weight=0.1) + EXP-3 (0.3). Identical EXCEPT:
#   --l3_diversity_weight 0.3 -> 0.5   (strongest diversity dose, 3-point sweep 0.1/0.3/0.5)
#   independent RUN / output_dir / log / master_port (29824)
# Runs on diskB REMOTE node 28.49.57.76 (8x H20, share_304376610 FS).
#   PROJECT_ROOT -> diskB share_304376610, PYBIN = diskB .venv/bin/python.
set -euo pipefail
...
RUN="exp5_l3div05"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29824 \
  ...
  --multi_query_tau 1.0 --l3_diversity_weight 0.5 --l3_diversity_threshold 0.5 \
