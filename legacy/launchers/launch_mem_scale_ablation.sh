#!/bin/bash
# Launch memory_scale ablation on the current node
# Usage: bash scripts/launch_mem_scale_ablation.sh <scale_value> <log_suffix>
set -e
...
torchrun --nproc_per_node=8 scripts/train_mem_space_pg19.py \
  ...
  --infini_beta_lr 0.1 \
  --infini_memory_scale_init ${SCALE} \
  --lr 1e-3 \
  --max_steps 3000 \
  --max_chunks 500 \
  --skip_chunks 0 \
  ...
  --output_dir outputs/mem_scale_${SCALE}
