#!/usr/bin/env bash
# ROUTE-A balance sweep arm4 (B200 28.89.18.188, 8x L20A 183GB).
# Algorithm = byte-for-byte identical to launch_routeA_arm4.sh (H20/diskA version),
#   which itself is P11 chunk512 + the ONLY change being:
#     --use_st_gumbel_topk --st_gumbel_temperature 1.0
#   (Gumbel noise on selection logits to force exploration of cold slots; touches only
#    the grad-free index path. loss_free/entropy/temp all kept at P11 defaults.)
#   total_steps 1000 + save_interval 500 (sweep length), every other P11 hyperparam intact.
# GOAL: lift usage_cov (~0.25, only ~32/128 slots used) via stochastic top-k selection.
#
...
#   2. unique master_port (29814) + isolated output_dir/log.
#   3. HF fully offline (B200 wzc1 has no direct internet) + woa proxy fallback.
#   4. batch: L20A 183GB; keep P11's bs4 x grad_accum1 x 8gpu = eff_batch 32 (Gumbel adds no mem).
set -euo pipefail
...
RUN="routeA_arm4_stgumbel"
mkdir -p logs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29814 \
