#!/usr/bin/env bash
# EXP-3 (2026-06-11): L3 summary-token diversity regularization, STRONGER weight arm.
# Companion arm to EXP-1 (l3_diversity_weight=0.1). Identical EXCEPT:
#   --l3_diversity_weight 0.1 -> 0.3   (stronger diversity dose)
#   independent RUN / output_dir / log / master_port (29822, avoids 29820/29821)
# Runs on LOCAL diskA node (8x H20, share_303098609 FS). PYBIN = .venv/bin/python.
set -euo pipefail
...
RUN="exp3_l3div03"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29822 \
  ...
  --multi_query_tau 1.0 --l3_diversity_weight 0.3 --l3_diversity_threshold 0.5 \
