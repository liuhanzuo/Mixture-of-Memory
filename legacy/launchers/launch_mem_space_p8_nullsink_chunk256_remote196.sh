#!/usr/bin/env bash
# P8 NULLSINK scale-up arm: chunk_size 256 (vs chunk128 on local node).
# Same code as local P8 nullsink (commit c69cd8d on 1f46b4d: null/sink slot +
# memory_xattn trainable+saved). Only knobs changed vs launch_mem_space_p8_nullsink.sh:
#   - chunk_size 128 -> 256 (scale-up dimension, orthogonal to the read-path fix)
#   - 8 GPUs x bs1 x ga4 = eff batch 32 (matches local 4x1x8=32 and original 8x1x4=32)
#   - total_steps 2000 -> 5000 (data is ~404M tok / 280k docs; 5000 steps still < 1 epoch,
#     no repeats; original 2000 only consumed ~6% of data)
# Runs on REMOTE disk-A node 28.59.80.196 (shared FS with local, no code sync needed).
# eval_interval=0 (inline BABILong -> NCCL SIGABRT). Offline eval on checkpoints is the judge.
set -euo pipefail
...
RUN="mem_space_perdoc_chunk256_p8_nullsink_r196"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29788 \
  ...
  --chunk_size 256 --batch_size 1 --num_slots 128 --top_k 16 --selector_dim 128 \
