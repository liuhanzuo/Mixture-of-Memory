#!/usr/bin/env bash
# Phase-1B v5 cold-start alpha — single-factor ablation over v2 L1+L3 baseline.
#
# v2 baseline (outputs/babilong_sft_phase1b_v2_10k/):
#   * 10k steps, lr=2e-5, lengths=1k,2k,4k uniform, pg19_mix=0.20
#   * L1+L3, dual gate, shared bank, random init noise=0.05
#
# v5 changes vs v2 (ONLY this flag):
#   1.  --zero_alpha_on_cold_start: on the first chunk of each sample, output
#       fusion alpha is forced to 0 so uninitialised slot content does NOT
#       pollute hidden states.  Selector + extended forward + writeback still
#       execute normally, so the first chunk's content populates the bank.
#   2.  NO v3 short-fix (no --skip_mem_when_short, no 0k resampling).
#   3.  NO L2.
#
# Launch: bash scripts/launch_phase1b_v5_coldstart_alpha.sh
# Outputs: outputs/babilong_sft_phase1b_v5_coldstart_alpha/
# Log:     logs/phase1b_v5_coldstart_alpha_<timestamp>.log
...
EXP_TAG="${EXP_TAG:-phase1b_v5_coldstart_alpha}"
...
# v2 architecture hyperparams (replicated exactly from v2 L1+L3 baseline)
