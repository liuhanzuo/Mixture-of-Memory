#!/usr/bin/env bash
# Writeback-mode arm B (diag_gate, remote H20-2 28.59.80.196): slot_dim=16384,
# 128 slots, per-feature diagonal gate (6*slot_dim params/layer) replacing the
# LM2 dual_gate. Single-variable comparison vs lowrank_gate (A) and scalar_beta (C).
set -euo pipefail
...
setsid bash -c "$PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29782 \
  ...
  --slot_dim 16384 --writeback_mode diag_gate \
