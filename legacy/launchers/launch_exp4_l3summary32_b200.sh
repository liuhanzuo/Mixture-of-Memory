#!/usr/bin/env bash
# EXP-4 (2026-06-11): REDUCE the L3 bottleneck (--l3_n_summary 64 -> 32).
# Reverse arm of EXP-2 (which widens 64 -> 128). EXP-D diagnosed that canonical P11
# L3 summary tokens collapse hard (l3_tok_cos=0.99, 64 tokens ~= 1 direction), so the
# token budget may be over-provisioned; cutting to 32 tests whether fewer tokens stay
# just as accurate (more focused / fewer params, no point drop). Together with
# EXP-2(128) + base(64) + EXP-4(32) this forms a 3-point l3_n_summary sweep.
# Identical to launch_exp2_l3summary128_diskB249.sh EXCEPT:
#   --l3_n_summary 128 -> 32 (shape change -> from-scratch train, no warm-start)
#   independent RUN / output_dir / log / master_port (29823)
#   B200 adaptation (mirrors launch_routeA_arm4_b200.sh):
#     PROJECT_ROOT -> B200 wzc1 disk; PYBIN -> wzc1 .venv (torch 2.10+cu128, L20A sm_100).
#     HF fully offline (B200 wzc1 has no direct internet) + woa proxy fallback.
# Runs on B200 node 28.89.18.188 (8x L20A 183GB). total_steps 1000, save_interval 500,
# eval_interval 0; every other P11 chunk512 hyperparam intact.
set -euo pipefail
...
RUN="exp4_l3summary32"
mkdir -p logs
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29823 \
  ...
  --dual_gate_tanh_new --use_l3_summary --l3_n_summary 32 --l3_n_layers 2 --l3_n_heads 8 \
