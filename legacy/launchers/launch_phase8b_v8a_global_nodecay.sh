#!/usr/bin/env bash
# Phase-8B v8-A — global slots with near-zero forget dual gate on Llama-3-8B-Instruct.
#
# Purpose: v8-A makes global slots use dual gate (not replace=True) but with a
# heavily biased forget gate (global_slot_forget_bias=-5.0 → sigmoid≈0.007),
# so global slots nearly never forget. Compared to v7 (hard replacement),
# this allows the model to LEARN whether to forget global slot content.
#
# Launch (on B200 28.89.17.144 via wzc1 shared FS):
#   bash scripts/launch_phase8b_v8a_global_nodecay.sh
#
# Target node: B200 28.89.17.144 (8x L20A, 183 GiB)
# Outputs: outputs/babilong_sft_phase8b_v8a_global_nodecay_<timestamp>/
# Log:     logs/phase8b_v8a_global_nodecay_<timestamp>.log
...
EXP_TAG="${EXP_TAG:-phase8b_v8a_global_nodecay}"
BACKBONE_PATH="${BACKBONE_PATH:-models/Meta-Llama-3-8B-Instruct}"
PYTHON_BIN="${PYTHON_BIN:-$PROJECT_ROOT/.venv/bin/python}"
NUM_GLOBAL_SLOTS="${NUM_GLOBAL_SLOTS:-8}"
GLOBAL_FORGET_BIAS="${GLOBAL_FORGET_BIAS:--5.0}"
...
# Same hyperparams as P8 champion recipe
...
CMD="$PYTHON_BIN -m torch.distributed.run --nproc_per_node=${NUM_GPUS} scripts/train_mem_space_babilong.py \
  ...
  --global_slot_forget_bias ${GLOBAL_FORGET_BIAS} \
  --use_l3_summary \
...
echo "[launch] backbone=${BACKBONE_PATH} num_global_slots=${NUM_GLOBAL_SLOTS} global_forget_bias=${GLOBAL_FORGET_BIAS}"
echo "[launch] ${CMD}"
