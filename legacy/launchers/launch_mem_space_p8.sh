#!/usr/bin/env bash
# P8 (dedicated memory cross-attention READ path) Dolmino CPT arm.
# Built from launch_mem_space_p7p9.sh (the P7 loss-free-balance + P9 register-
# slots arm with stable, learnable routing). The ONLY functional difference is
# the memory READ path:
#   P8: --use_memory_xattn  → slots get a DEDICATED cross-attention with their
#       OWN softmax + a per-head content-dependent gate that is ACTIVE at init
#       (gate≈0.4, out_proj small-random). This removes the ~0.2% attn-mass
#       dilution of the KV-prepend path (researcher P2 root cause). Routing
#       (P7) is already fixed; P8 is what gives that routing a gradient to learn
#       from. Reference designs: YOCO (2405.05254), Memorizing Transformers
#       (2203.08913), Infini-attention (2404.07143).
# Everything else (P7 loss-free balance, P9 register slots, route_aux=1.0,
# slot_query routing, dual_gate writeback, L3 summaries, chunk128) is IDENTICAL
# to p7p9 so the comparison is clean: P8 vs P7P9 isolates the read path.
# eval_interval=0 (inline BABILong eval -> NCCL SIGABRT, CODEBUDDY.md). Offline
# BABILong on the checkpoint is the judge.
# Designed for a FULLY-FREE 8-GPU disk-A node (local 29.162.227.178 OR remote 28.59.80.196).
set -euo pipefail
...
RUN="mem_space_perdoc_chunk128_p8"
setsid bash -c "$PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29784 \
