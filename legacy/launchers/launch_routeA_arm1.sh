#!/usr/bin/env bash
# ROUTE-A balance sweep arm1: identical to launch_mem_space_p11_chunk512_remote196.sh
# EXCEPT --loss_free_update_rate 0.001 -> 0.01 (+ total_steps 1000 for the sweep).
# Goal: lift usage_cov (~0.25, only ~32/128 slots used) via faster loss-free bias update.
set -euo pipefail
...
RUN="routeA_arm1_lossfree001"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $PYBIN -m torch.distributed.run --nproc_per_node=8 --master_port=29810 \
