...
Our literature audit found no prior reusable-context system that jointly treats
transformer split depth as a tunable serving variable and isolates its
incremental effect with an identical-evidence $j{=}0$ endpoint; CoMem is the
first such formulation to our knowledge. Earlier activation-reuse methods cache a
chosen intermediate representation for repeated downstream runs
~\citep{readonce,embeddingrecycling}, HCache stores hidden states at every layer
to reconstruct layer-wise KV~\citep{hcache}, and PIC/modular-cache systems
store or learn layer-wise KV objects~\citep{cacheblend,epic,kvpacket,cartridgesbase}.
CoMem instead stores one residual per token at the chosen split and directly
continues the decoder suffix. This formulation makes depth itself measurable
...
1.10\,s ($64.9\times$) and peak memory from 50.0 to 18.7\,GB. These two
numbers answer different questions: $1.403\times$ isolates prepaid depth;
$64.9\times$ measures the composed bounded-read path.

CoMem further provides, to our knowledge, the first unified measurement of a
...
use different backbones and measurement boundaries, so we compare their
interfaces conceptually rather than claim an unmatched empirical win. The full
set of deployment, generalization, and statistical limits is consolidated in
...
$1.403\times$ matched Read speedup and a $64.9\times$ composed 128k prefill
point.
