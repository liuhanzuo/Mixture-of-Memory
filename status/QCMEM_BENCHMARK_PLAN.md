...
> - **⚠️⚠️ BABILong Qwen3-scale 结果（2026-07-14 跑）在 thinking-fix（`30bb2ab`，2026-07-16）之前生成 → thinking/MC 污染，长档分被压低**：抽查 8B-adapter qa1 原始输出，~90% 带 thinking/MC 标记（"Okay,"/"let's tackle"/"Choices: A.B.C"）；**8k/16k 答案仍在首句（首句命中 65/56 ≈ 官方 64/55，影响小），但 32k 答案在首句仅 23% 而出现在输出任意处 57%** —— 模型长档先 ramble/thinking 把答案挤出首句，官方首句判分判错 → **32k=21 是污染压低值，非干净长程效应，真值估计 35–50**。**待办：用 `--enable_thinking False` 重跑 8B（并核 4B/1.7B/0.6B 是否同批污染）+ 确认 chat_template_no 配置下 thinking 真被抑制，再重判填表。**
