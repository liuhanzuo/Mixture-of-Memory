...
**最后实测 2026-08-17 06:07 GMT+8（heartbeat）。32/40 卡占用，LOCAL 8 卡空闲（**故意**，理由见表下）。每节点 8 个 compute PID = 单一主人，无抢卡。**
...
| `.212` | 8×B200 sm_100 | wzc1 | `olmo2_probe2_7B_keep14fresh2_distill` | ~52500+/200000 | 157.8 GB | 100% | **2.4475–2.4510**（连续 4 个 ckpt 区间） | 2.4500 | healthy 1.000× — ETA 08-21 10:16 |
| `.73` | 8×H20 sm_90 | zwfy6 | `olmo2_probe2_7B_keep12fresh2` + `chain_keep12_eval_200k` watcher | 193500/200000 | 96.4 GB | 100% | **7.500**（watcher 4 个连续 5-min 区间，每区间 40 步，4/4 全等） | 7.9160 | **0.947× — 比基线快 5.3%**，ETA **~19:33** |
...
| `.104` | 8×H20 sm_90 | zwfy6 | `paperC_qwen3base_heal_k8f2` | 68500/200000 | 78.8 GB | 100% | **5.8363–5.8518**（连续 3 个 ckpt 区间） | 5.8380 | healthy 1.001× — ETA 08-26 03:33 |

> **⚠️ 自我纠正（06:07 覆盖 05:52 的记载）**：上一轮我把 `.73` 记成 **8.33 s/step / 1.052× / ETA ~21:05**。**那是错的。**
> 我拿 `05:21:27 step193200 → 05:46:27 step193380`（跨 5 个区间）相除得 Δstep=180；但 watcher 是**固定 5 分钟一行、
> 每行 40 步**，5 个区间必然是 **200 步**，所以两个端点里至少有一个被 watcher 自己的 `grep -oE 'step [0-9]+'`
> 粒度截断了 20 步。改用**同源相邻行**逐区间算：7.50 / 7.50 / 7.50 / 7.50 —— 4/4 完全相等，
> 真实值 **7.500 s/step，比基线快 5.3%**，ETA 从 21:05 提前到 **19:33**。
> **教训**：跨越多个采样区间做除法会把量化误差放大成假的「变慢」；判速率必须用相邻同源样本，并报窗口内每个区间。
