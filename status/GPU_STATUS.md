...
## 🔄 2026-08-16 20:30 — **40/40 全忙**；★ 在 `.73` 挂上 keep12→eval 的 **chain watcher**（本轮唯一操作）

| 节点 | 卡 | 任务 | step | loss | compute 速率 | 状态 |
|---|---|---|---|---|---|---|
| LOCAL | 0-7 | Paper B keep10fresh2 | 178600/200000 | 2.4782 | **1.2000** = 基线 | ▶️ 健康 |
| `.212` | 0-7 | Paper B #99 keep14-distill | 38560/200000 | 2.7046 | **2.3500** ≈ 自报 2.36 | ▶️ 健康 |
| `.73` | 0-7 | Paper B keep12fresh2 | 189140/200000 | 2.4134 | **7.8000** | ▶️ 健康，**剩 10860 步（约 24 h）** |
| `.82` | 0-7 | Paper B keep8fresh2 | 162260/200000 | 2.5909 | **5.8000** | ▶️ 健康 |
| `.104` | 0-7 | paperC qwen3base_heal_k8f2 | 62760/200000 | 2.7425 | **5.7500** | ▶️ 健康（勿动） |

每节点 8 个 compute PID，无抢卡，0 空卡，全部 log 零 error 行。Monitor http200 OK。

### ★ 本轮操作：`.73` 挂上 chain watcher（`scripts/chain_keep12_eval_200k.sh`）

**问题**：keep12 约 24 h 后到 200k，而 **LOCAL 和 `.73` 都没有任何 watcher**
（`pgrep -af 'watch|chain|eval_paperb'` 两侧皆空）。跑完 = 8 张 H20 空转等 heartbeat 发现。

**触发条件是 ckpt 文件而非 log 行**：log 打出 step200000 可能早于 43.9 GB 写完，而
`eval_paperb_ladder_200k.sh` 会断言 ckpt 存在 —— 按 log 触发就会把半个文件交给它。
watcher 等「路径存在 **且** 连续两次轮询 size 不变」。

**部署前用真实文件 I/O 验过两个方向**（"等 size 稳定" 正是那种会静默提前触发的规则）：

| 场景 | 结果 |
|---|---|
| 文件在整个窗口内持续增长 | rc=124，8 条 `still writing`，**0 次启动** |
| 文件写完且静止 | 2 次稳定轮询后 rc=0，driver 恰好被调用 1 次 |

> ⚠️ 我第一次的测试**自己错了**：生成器在 timeout 之前就写完了，文件变静止、watcher 正确触发，
> 我却读成「提前触发」。改成让增长持续超过窗口才是有效对照。**setup 早于窗口结束的对照不是对照。**

部署细节：跨盘 `scp -O` + md5 核对一致（`0076ff4b88f75723daf69af770e096d2`）；
放在 `.73` 因为 ckpt 是 zwfy6-resident 且 driver 的 `REQUIRE_SM=9.0` 对应 H20。
**从另一条 ssh 连接**确认存活（3 个 PID、poll=300s、读到真实 step 189180、41 个 trainer 进程未受影响）。
启动那条 ssh 返回 143 是我自己的 `pgrep` 落在它的进程组里，**不是启动失败** —— 所以核查必须换连接做。
它不 kill 任何东西、不挑节点、driver 只跑一次、48 h 未见完整 ckpt 就 rc=3 退出而不是对着卡死的 run 启动。
