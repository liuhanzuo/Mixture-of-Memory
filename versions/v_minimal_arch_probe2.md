...
| scratch14（14层随机初始化） | 训练中（step1560=50.66，~3.5h 到 16000） | — |
| base（全16层充分训练） | 25.34 | — |

- **两臂都明显反超全16层 base**：armB(全训)-23%、armA(冻结前12只训2新层)-5.4%。
- **★结论（待 scratch14 收敛盖章）**：前12层表征直接可复用（armA 冻结也反超）+ 顶部4冗余层换成2 fresh 层重学 NTP → **不仅不损失、反而更优**。这支持"顶层冗余可精简"的强假设。
- **关键待完成对照 scratch14@16000**：若 scratch14 收敛后 ≫ armB（接近或劣于 base）→ 证明 armB 的优势来自"继承前12"而非"14层这个深度本身"（方向4 因果坐实）；若 scratch14 ≈ armB → 优势主要是深度效应（削弱结论）。**判决取决于此**。
- ⚠️ armB<base 可能部分因 14 层比 16 层更易训（同步数下浅模型收敛快）——scratch14@16000 正是隔离这一 confound 的对照。
