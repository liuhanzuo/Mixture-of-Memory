#!/usr/bin/env bash
# diskA build of the ISOLATED S5 package tree. Materialize the PRISTINE ANCHOR
# (commit d963e50 = llama_mem md5 99631a8) llama/ package into a physically
# separate tree, apply the S5 single_layer_mem patch, then CPU regression smoke.
# Anchor pulled from commit d963e50 explicitly (NOT HEAD — HEAD is now the S4b
# commit a699e3e, which would (a) not match the S5 diff context and (b) defeat
# the single-axis isolation). CPU-only, no GPU.
set -uo pipefail
R=/apdcephfs_zwfy6/share_303098609/pighzliu_code/Mixture-of-Memory
NESTED=$R/external/landmark-attention          # nested git repo
ANCHOR_REV=d963e50                             # pristine anchor (md5 99631a8)
S5TREE=$R/external/landmark_s5_tree            # <-- isolated S5 package dir
ANCHORTREE=$R/external/_s5_anchor_ref          # pristine anchor reference copy
PATCHDIR=$R/external/landmark/s5_patch
PY=$R/external/landmark_venv/bin/python

echo "===== [0] sanity: anchor rev md5 (expect 99631a8...) ====="
( cd "$NESTED" && git show ${ANCHOR_REV}:llama/llama_mem.py | md5sum )

rm -rf "$S5TREE" "$ANCHORTREE"
mkdir -p "$S5TREE/llama" "$ANCHORTREE/llama"

echo "===== [1] materialize PRISTINE anchor llama/ into isolated tree (rev ${ANCHOR_REV}) ====="
( cd "$NESTED" && git archive ${ANCHOR_REV} llama/ | tar -x -C "$S5TREE" )
( cd "$NESTED" && git archive ${ANCHOR_REV} llama/ | tar -x -C "$ANCHORTREE" )
...
echo "===== [3] ANCHOR forward (pristine) ====="
( cd "$ANCHORTREE/llama" && "$PY" s5_smoke_worker.py none "$ANCHORTREE/anchor_none.pt" ) || { echo "ANCHOR RUN FAILED"; exit 21; }
