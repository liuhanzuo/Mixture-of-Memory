...
> - ⚠️⚠️ **~~Qwen base 路径名错~~ 该条已作废（2026-08-12 两盘实测纠正）**：旧文写「`models/Qwen3-8b-local` 在 zwfy6 不存在」是**错的**，已误导多个 agent。实测该 symlink **两盘都存在**：wzc1 `models/Qwen3-8b-local → /apdcephfs_wzc1/.../models/Qwen--Qwen3-8b`，zwfy6 `→ /apdcephfs_zwfy6/.../models/Qwen--Qwen3-8b`。harness 默认 `--model_path models/Qwen3-8b-local` 可直接用，**无需 override**。
>   **真正的坑是它指向 Instruct 而不是 Base**：`Qwen--Qwen3-8b` = Qwen3-8B-**Instruct**（`eos_token_id=151645` `<|im_end|>`、ctx 40960、README `base_model: Qwen3-8B-Base`）；真 Base 在 `models/Qwen3-8B-Base`（`eos=151643`、ctx 32768），两盘都有（wzc1 自 2026-07-14，zwfy6 自 2026-08-11 22:02）。**两者 5 个 shard 字节数完全相同但权重不同**（实测 shard1 前 400MB md5 不同、`model.safetensors.index.json` 不同）→ **不能靠目录大小/文件名判断，必须查 `config.json` 的 `eos_token_id`**。任何「Qwen3-8B base arm」claim 都要核 `arch_meta.json` / `summary.json` 里 `base_model` 的实际字符串。
